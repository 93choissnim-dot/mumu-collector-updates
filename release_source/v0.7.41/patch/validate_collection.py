...
class CollectionRegression(unittest.TestCase):
    ...
    def test_pending_facility_after_restart_never_repeats_input(self):
        c,d,clock,_=self.run_flow()
        c.action_state.reserve('farm')
        self.assertEqual(self.cycle(c,clock)['farm'],'deferred')
        self.assertEqual(d.claims['farm'],0)
        self.assertTrue(c.action_state.pending('farm'))
    def test_pending_facility_empty_can_reconcile_without_input(self):
    ...
    def test_active_after_input_does_not_authorize_second_claim(self):
        c, d, clock, seen = self.run_flow(post={"farm": ["ready", "empty"]})
        self.assertEqual(self.cycle(c, clock), {"farm":"deferred","wood":"collected","mine":"collected"})
        self.assertEqual(d.claims, dict.fromkeys(LABELS,1))
        self.assertEqual(seen.count(("farm", "collected")), 0)

    def test_active_after_input_never_spends_three_attempts(self):
    ...
    def test_facilities_with_unconfirmed_inputs_are_deferred_independently(self):
        c, d, clock, _ = self.run_flow(post={"wood": ["ready", "empty"],
                                            "mine": ["ready", "ready", "empty"]})
        self.assertEqual(self.cycle(c, clock), {"farm":"collected","wood":"deferred","mine":"deferred"})
        self.assertEqual(d.claims, dict.fromkeys(LABELS,1))

    def test_still_active_after_one_click_continues_without_false_success(self):
        c, d, clock, _ = self.run_flow(post={"farm": "ready"})
        result = self.cycle(c, clock)
        self.assertEqual(result, {"farm": "deferred", "wood": "collected", "mine": "collected"})
        self.assertEqual(d.claims, dict.fromkeys(LABELS,1))
    ...
    def test_unreadable_button_on_known_page_does_not_block_departure(self):
        ...
        result = self.cycle(c, clock)
        self.assertEqual(result["farm"], "deferred")
        self.assertEqual(d.claims, dict.fromkeys(LABELS, 1))
        self.assertEqual(d.state, "menu")
