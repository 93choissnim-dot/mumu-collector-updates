...
from diagnostics import save_collection_failure,save_execution_trace,save_input_evidence
    ...
    def launch(self,mode,targets=None):
        ...
        try:
            ...
            for ident,p in self.players.items():
                ...
                if not job['rooms']:continue
                job['minutes']=60 if mode=='daily' else float(p['minutes'])
        ...
        def execute(job,rooms):
            ...
            try:
                ...
                if ledger:
                    def input_evidence(phase,task,step,detail):
                        try:
                    ...
                    ledger.on_input_evidence=input_evidence
