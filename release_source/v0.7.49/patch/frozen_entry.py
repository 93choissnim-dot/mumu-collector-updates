...
def health_check(output,full_ui=True):
    ...
    suite=unittest.TestSuite(unittest.defaultTestLoader.loadTestsFromTestCase(case)
        for case in (HistoryCountTests,PendingHistoryTests,RaidRetryTests,InputProvenanceTests,InputEvidenceTests,NativeAutumnTests,NativeNavigationTests,RetryResolutionTests,HistoryLifecycleTests,RewardRecoveryTests,UpdateDiagnosticsTests,
    ...
    output.write_text(json.dumps({'ok':True,'version':VERSION,'frozen':bool(getattr(sys,'frozen',False)),
        ...
        'daily_native_color':True,'worldboss_recovery':True,'daily_step_recovery':True,'diagnostic_trace':True,'autumn_rewards':True,'raid_repeat_recovery':True,'pending_history_counts':True,'input_provenance':True,'input_evidence':True,'autumn_native_zero':True,'native_navigation':True,'input_safety':True,'pass_icon_identity':True,'efficiency_safety':True,
