...
def export_diagnostics(data, target):
    ...
    for task,steps in DAILY_STEPS.items():
        ...
        allowed.update(p.name for p in data.glob('last_*_input.zip') if re.fullmatch(
            rf'last_[0-9a-f]{{24}}_{task}_(?:{step_pattern})_(?:requested|resolved)_input\.zip',p.name))
...
def save_input_evidence(data,ident,scope,task,step,phase,detail,image,screen,*,run_id=None):
    """Two replaceable local archives per account/step, linked by request id.
    ...
    if (not re.fullmatch('[0-9a-f]{24}',str(ident)) or
        ...
        step not in DAILY_STEPS.get(task,()) or phase not in {'requested','resolved'}):
