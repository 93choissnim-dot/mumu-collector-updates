...
class FleetCollection:
    def launch(self,mode,targets=None):
        ...
        def execute(job,rooms):
            ...
            ident=job['id'];scope=account_scope(ident,job.get('daily_profile',''));serial=None;collector=None;results={};ledger=None
            ...
            try:
                ...
                if ledger and mode=='retry':
                    for room in rooms:
                        if room in DAILY_TASKS:ledger.reset_blocked(scope,room)
                for room in list(rooms):
                    ...
                    if room in DAILY_TASKS and ledger.done(scope,room):
