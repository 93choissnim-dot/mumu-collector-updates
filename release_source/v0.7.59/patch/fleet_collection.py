    ...
    def launch(self,mode,targets=None):
        ...
        try:
            ...
            for ident,p in self.players.items():
                ...
                if mode=='resume':
                    ...
                    allowed=set(selected_tasks(p))|set(DAILY_TASKS)
        ...
        def execute(job,rooms):
            ...
            def recorded(room,result,reason=None):
                ...
                if journal is not None:journal.result(scope,room,result)
            ...
            try:
                ...
                if mode in {'daily','retry'} and 'daily_dungeons' in rooms:collector.arm_dungeon_resume()
                if mode in {'daily','retry'} and 'daily_guild' in rooms:collector.arm_guild_resume()
            ...
            if mode!='verify_daily':
                ...
                if mode=='resume':
                    for job in jobs:
                        ...
                        current=set(journal.remaining(scope))
                        job['rooms']=[r for r in job['rooms'] if r in current]
                    jobs[:]=[job for job in jobs if job['rooms']]
                for job in jobs:
                    scope=account_scope(job['id'],job.get('daily_profile',''))
                    if mode=='daily':
                        ManualQuestLedger(DATA/'daily_manual.json').begin_run(scope,legacy_path=DATA/'daily_tasks.json')
