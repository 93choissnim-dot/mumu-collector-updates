...
class Dashboard:
    def build(self):
        ...
        self.more_menu.add_command(label='미완료 작업 이어하기',command=self.resume_unfinished)
        ...
        label(info,'  이번 실행 수행 확인 ',size=11,color=MUTED).pack(side='left');self.count_label=label(info,'0건',size=11,color=MINT);self.count_label.pack(side='left')
        ...
        self.once_button=button(footer,'일반 작업 1회',lambda:self.launch('once'),width=120,height=44)
        self.start_button=button(footer,'자동 수령 시작',lambda:self.launch('repeat'),primary=True,width=158,height=44)
        self.daily_button=button(footer,'던전 / 길드 / 패스',lambda:self.launch('daily'),width=156,height=44,border_color=GOLD,border_width=1)
        from ui_roster import tooltip
        self.daily_button._tooltip_text='패스 → 일일 던전 7종 → 길드 순차 실행\n무료 항목만 실행하며 유료 구매와 루비 기부는 하지 않습니다.\n누를 때마다 현재 게임 상태를 확인합니다.'
    ...
    def apply_layout(self):
        ...
        compact=width<1100;short=height<580;narrow_header=width<800;multi=len(self.players)>1;empty=not self.players;busy=self.busy()
        ...
        self.identity_panel.grid_configure(row=0,column=0,columnspan=2 if narrow_header else 1,padx=(18,10),pady=(8,4) if narrow_header else (10,10))
        self.toolbar_actions.grid_configure(row=1 if narrow_header else 0,column=0 if narrow_header else 1,columnspan=2 if narrow_header else 1,sticky='e',padx=(12,16),pady=(0,6) if narrow_header else 0)
    ...
    def guide_dialog(self):
        ...
        for title,text in [('01  뮤뮤 연결','게임을 실행하고 뮤뮤 연결을 누릅니다. 목록의 이름을 누르면 해당 화면과 기록을 볼 수 있습니다.'),('02  뮤뮤별 작업 설정','각 뮤뮤의 설정에서 수령할 작업과 간격을 선택합니다. 수련은 재료를 사용하는 작업입니다.'),('03  실행과 제어','체크된 뮤뮤만 순서대로 실행합니다. 일반 작업 1회와 자동 수령은 선택한 일반 작업을 실행합니다. 던전 / 길드 / 패스는 무료 일일 작업을 실행합니다.'),('04  중지와 이어하기','일시중지 후 재시작하면 이어서 진행합니다. 종료 후에는 더보기의 미완료 작업 이어하기를 사용하세요. 같은 계정의 당일 기록만 이어갑니다.')]:
