...
class RosterUI:
    ...
    def refresh_details(self,summaries=None):
        ...
        limit=32 if width<800 else max(6,min(22,int((width-410)/24)))
        ...
        issues=sum(e.get('result') in ISSUES for t,e in entries.items() if p and (t in selected_tasks(p) or t in DAILY_LABELS))
        attention=('진행 중: '+TASK_LABELS[active]) if active in TASK_LABELS else (f'확인 필요 {issues}개 / 결과를 누르면 원인을 볼 수 있습니다.' if issues else '')
        if attention:
