...
def build():
    ...
    feed={'app_id':'ChankiHelperExe','protocol':1,'version':VERSION,
        ...
        'sha256':sha(archive),'size':archive.stat().st_size,'notes':'최소화 후 작업표시줄 유지 및 창 복원 개선'}
...
def smoke(exe,result):
    ...
    assert report['ok'] and report['frozen'] and report['version']==VERSION and report['bits']==64 and report['full_ui_checks'] and report['game_theme_ui'] and report['improvements_ui'] and report['taskbar_ui'],report
