...
class Collector:
    ...
    def top_bar_tap(self,screen,key,*,validate=None):
        ...
        marker='top_event' if key=='pass' else 'top_'+key
        deadline=self.now()+5
        while marker not in screen.matches:
            ...
            screen=fresh
        if key=='pass':
            # Direct pass identity needs a separately approved reference. Require
            # the known main layout and its adjacent event icon before navigation.
            self.tap(screen,point=(599,28),required=(marker,),validate=validate,action='open_pass')
        else:self.tap(screen,marker,validate=validate,action='open_'+key)
        self.pause(.6)
    ...
    def click_match(self, state, name, screen=None, *, before_input=None):
        ...
        self.tap(screen,name,before_input=before_input)
        self.pause(.6)
    ...
    def wait_for_room(self, room, *, allow_exit=False, require_button=False, entry=False, timeout=20):
        deadline = self.now() + timeout
    ...
    def ensure_menu(self, screen=None, room=None):
        ...
        if screen.state == "main":
            ...
            # Do not toggle the button repeatedly if the menu fails to open.
    ...
    def check_claim_button(self, room, screen=None):
        ...
        for check in range(1, self.BUTTON_CHECKS+1):
            if check != 1 or screen is None or screen is not self.last_screen or not self.room_visible(screen, room):
                screen = self.wait_for_room(room)
    ...
    def collect_room(self, room, screen=None):
        ...
        self.claim_counts[room] = getattr(self, 'resume_counts', {}).pop(room, 0)
        screen = self.check_claim_button(room, screen)
        initially_empty = "empty_"+room in screen.matches and "ready_"+room not in screen.matches
        empty = initially_empty
        saw_ready = self.claim_counts[room] > 0
        confirmed = saw_ready and empty
        for attempt in range(self.claim_counts[room]+1, self.CLAIM_ATTEMPTS+1):
            if empty:
            ...
            self.progress(f"{LABELS[room]}: 수령 버튼 누르기 ({attempt}/{self.CLAIM_ATTEMPTS})")
            self.click_match(screen.state, button, screen, before_input=lambda:self.claim_counts.__setitem__(room,attempt))
            ...
            empty = "empty_"+room in screen.matches and "ready_"+room not in screen.matches
            confirmed |= saw_ready and empty
            if empty:
                self.log(LABELS[room] + ": 비활성 전환 확인 → 남은 재클릭 생략")
                break
            if attempt < self.CLAIM_ATTEMPTS and self.claim_button(screen, room):
                self.log(LABELS[room] + ": 수령 완료가 확인되지 않아 재시도합니다.")
        result = "collected" if confirmed else ("skipped" if initially_empty and empty else "attempted")
        if result == "attempted" and self.claim_counts[room] == 0:
            result = "unrecognized"
        self.results[room] = result
        detail = {"collected": "수령 완료 확인", "skipped": "수령할 자원 없음 확인",
                  "attempted": "수령 버튼 누름 / 완료 여부 미확인",
                  "unrecognized": "수령 버튼 미인식 / 이번 시설 보류"}[result]
        self.log(f"{LABELS[room]}: {detail} / {self.claim_counts[room]}회 클릭")
        if self.on_result:
            self.on_result(room, result)
        if self.on_issue and result in {"attempted", "unrecognized"}:
