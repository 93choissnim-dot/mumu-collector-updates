...
def health_check(output,full_ui=True):
    ...
    top=vision.recognize(composite(vision,'main_menu','main_character','main_pet','top_event','top_worldboss'))
    assert all('top_'+k in top.matches for k in ('event','worldboss'))
    ...
    output.write_text(json.dumps({'ok':True,'version':VERSION,'frozen':bool(getattr(sys,'frozen',False)),
        ...
        'daily_native_color':True,'worldboss_recovery':True,'daily_step_recovery':True,'diagnostic_trace':True,'autumn_rewards':True,'input_safety':True,
