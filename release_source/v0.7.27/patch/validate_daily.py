...
class PassNavigationTests(unittest.TestCase):
    ...
    def run_route(self,wrong_entry=None,already=False):
        ...
        class Device:
            ...
            def capture(self):
                self.clock+=.1
                if self.page=='main':return composite(v,*MAIN_MARKERS,'top_event')
                im=np.full((540,960,3),110,np.uint8)
...
class GuardTests(unittest.TestCase):
    ...
    def test_navigation_survives_animated_icon_background(self):
        for key,point in [('pass',(599,28)),('dungeon',(509,505)),('guild',(563,505))]:
            c=self.collector();main=self.screen('main');main.matches['top_event']=Match('top_event',1,0,(599,28));c.daily_main=Mock(return_value=main)
...
class CompletedOutcomeTests(unittest.TestCase):
    ...
    def test_zero_entries_with_free_key_still_claims_and_enters(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            page='daily_room_stone';s=self.screen(page);c.daily_open=Mock();c.daily_wait=Mock(return_value=s)
            ...
            calls=[]
            def tap(screen,key=None,point=None,work=False):
    ...
    def test_verified_guild_rewards_already_claimed_do_not_collect_again(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            c.daily_guild_root=Mock(side_effect=[menu,menu,battle]);c.daily_ready=Mock(return_value=self.screen('daily_relic','relic_empty'))
