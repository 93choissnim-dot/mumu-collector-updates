...
from ui_theme import BG,PANEL,INSET,LINE,TEXT,MUTED,GOLD,BLUE,TONES,font,label,button
...
class RosterUI:
    def summaries(self,working=None):
        ...
        now=self.stop.clock()
        return {ident:player_summary(p,self.fleet_states.get(ident,{}),ident in live,busy,self.stop.paused,now)
                for ident,p in self.players.items()}

    def render_roster(self,force=False,working=None):
        if not hasattr(self,'roster_body'):return
        now=time.monotonic()
        if not force and now-self.ui_last_refresh<.35:return
        ...
        if signature!=self.roster_signature:
            ...
            for i,ident in enumerate(ids):
                row=ctk.CTkFrame(self.roster_body,fg_color=INSET,border_width=1,border_color=LINE,corner_radius=8)
                row.grid(row=i,column=0,sticky='ew',padx=3,pady=4);row.grid_columnconfigure(1,weight=1)
