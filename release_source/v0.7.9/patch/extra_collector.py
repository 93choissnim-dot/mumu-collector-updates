...
from task_catalog import EXTRA_LABELS,PAGE_MARKERS
from world_boss import WorldBossActions

class ExtraCollector(WorldBossActions,Collector):
    ...
    def cycle(self,selected,restore_sleep=False):
        ...
        for task in selected:
            if task not in LABELS and task not in EXTRA_LABELS:raise Halt('지원하지 않는 작업입니다.')
            title=LABELS.get(task) or EXTRA_LABELS[task]
            ...
            try:
                ...
                screen=self.screen()
                known=screen.state in set(PAGE_MARKERS)|{'main','menu'} or (task in LABELS and self.room_visible(screen,task))
                if not known:raise
