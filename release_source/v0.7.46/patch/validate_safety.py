...
class SafetyTests(unittest.TestCase):
    ...
    def test_autumn_repeated_failure_blocks_only_that_task(self):
        ...
        d.state='main'  # Repeat the same failed entry state.
...
class PreinputRecoveryTests(unittest.TestCase):
    def collector(self,states):
        c=Collector(Mock(),Mock(),threading.Event(),Mock());c.screen=Mock(side_effect=states);c.pause=Mock()
        return c
    def test_transient_unknown_requires_two_recognized_frames_before_any_input(self):
        s=Screen('ranking',{'x_rank_active':Match('x_rank_active',1,0,(843,114))});unknown=Screen('unknown',{})
        c=self.collector([unknown,s,s]);reserve=Mock()
        c.tap(s,'x_rank_active',before_input=reserve)
        self.assertEqual(c.screen.call_count,3);reserve.assert_called_once();c.device.click.assert_called_once_with((843,114))
    def test_partial_recovery_or_different_page_never_authorizes_input(self):
        s=Screen('ranking',{'x_rank_active':Match('x_rank_active',1,0,(843,114))});u=Screen('unknown',{})
        for frames in ([u]*7,[u,s,u,s,u,s,u],[u,Screen('reward',{})],[u,Screen('ranking',{})]):
            with self.subTest(frames=len(frames)):
                c=self.collector(frames);reserve=Mock()
                with self.assertRaises(Halt):c.tap(s,'x_rank_active',before_input=reserve)
                reserve.assert_not_called();c.device.click.assert_not_called()
    def test_stop_during_animation_wait_never_inputs(self):
        s=Screen('menu',{});c=self.collector([Screen('unknown',{}),s,s])
        def pause(_):c.stop.set();raise Halt('stop')
        c.pause=pause
        with self.assertRaises(Halt):c.tap(s,point=(20,20))
        c.device.click.assert_not_called()
    def test_midnight_during_animation_wait_never_inputs(self):
        s=Screen('menu',{});c=self.collector([Screen('unknown',{}),s,s])
        validate=Mock(side_effect=[None,Halt('midnight')])
        with self.assertRaises(Halt):c.tap(s,point=(20,20),validate=validate)
        c.device.click.assert_not_called()
