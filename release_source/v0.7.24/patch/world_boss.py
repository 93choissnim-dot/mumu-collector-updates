...
class WorldBossActions:
    def open_boss_selection(self,menu):
        ...
        if menu.state!='menu':raise Halt('월드보스: 진입 전 메뉴 확인이 필요합니다.')
        fresh=self.screen()
        if fresh.state!='menu':raise Halt('월드보스: 화면이 바뀌어 진입을 보류합니다.')
        self.device.click(TOP_BAR_POINTS['worldboss']);self.pause(.6)
