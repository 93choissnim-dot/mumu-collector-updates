...
class DailyExecution:
    def daily_needs_completion_review(self,step):
        if (self.daily_task!='daily_dungeons' or
            ...
            return False
        if getattr(self,'daily_verification_only',False):return True
        proof=self.daily_detail(step).get('completion_evidence')
        return not (isinstance(proof,dict) and proof.get('revision')==1 and
                    proof.get('kind')=='no_entries' and type(proof.get('observations')) is int and proof['observations']>=2)
    ...
    def daily_run_step(self,step,label,action):
        ...
        if review:
            ...
            # The first retry only inspects this formerly completed step.
        ...
        if ((self.daily_task,step)==('daily_guild','dungeon') and prior.get('status')=='blocked'
                and prior.get('reason') in {
                    ...
                    '길드 전리품 개수 확인이 필요합니다.'}
                and not prior.get('pending') and prior.get('guild_dungeon_rule_revision',0)<1):
            self.daily_checkpoint('pending',failures=0,fingerprint='',guild_dungeon_rule_revision=1)
            prior=self.daily_detail()
        ...
        if (prior.get('status')=='blocked' and repaired_reason and not prior.get('pending')
                and prior.get('recognition_revision',0)<40):
            self.daily_checkpoint('pending',failures=0,fingerprint='',recognition_revision=40)
            prior=self.daily_detail()
