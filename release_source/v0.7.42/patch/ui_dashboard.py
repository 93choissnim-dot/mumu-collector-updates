...
class Dashboard:
    def build(self):
        ...
        identity.grid(row=0,column=0,sticky='ew',padx=(24,10),pady=(14,12));identity.grid_columnconfigure(1,weight=1)
        label(identity,'',image=icon('brand',color='#C9A660',size=38),width=44).grid(row=0,column=0,rowspan=2,padx=(0,12))
        self.detail_name=heading(identity,'뮤뮤를 연결하세요',size=24,color=CREAM,anchor='w')
        ...
        self.connection_badge.grid(row=1,column=1,sticky='w')
        ...
        for key,title in TASK_LABELS.items():
            row=ctk.CTkFrame(self.detail_tasks,fg_color=INSET,corner_radius=2);row.grid_columnconfigure(1,weight=1)
