...
class RosterUI:
    def summaries(self,working=None):
        ...
        for ident,p in self.players.items():
            ...
            state['history_issue']=any(value in ISSUES for value in recorded.values())
    ...
    def render_roster(self,force=False,working=None):
        ...
        if signature!=self.roster_signature:
            ...
            for i,ident in enumerate(ids):
                ...
                check=ctk.CTkCheckBox(row,text='',variable=enabled,width=22,height=24,checkbox_width=18,checkbox_height=18,fg_color=ACCENT,checkmark_color=CREAM,border_color=MUTED,command=lambda key=ident:self.toggle_player(key))
        ...
        update(self.daily_button,state='normal' if daily_available and not blocked else 'disabled')
        ...
        if busy and self.pause_allowed:hint='일시중지 중' if self.stop.paused else ('자동 수령 중' if self.run_mode=='repeat' else '일일 퀘스트 수령 중' if self.run_mode=='daily' else '한 번 수령 중')
    ...
    def refresh_details(self,summaries=None):
        ...
        entries=self.history_entries(ident)
        stamps=[]
        for task,widget in self.detail_results.items():
            ...
            update(self.detail_times[task],text=short_time(entry.get('checked_at')) if entry.get('checked_at') else '—')
            update(self.detail_task_rows[task],fg_color=TINTS[tone] if tone in {'error','warning'} else INSET)
