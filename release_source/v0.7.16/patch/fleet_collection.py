...
from daily_state import DailyLedger,DAILY_TASKS
    ...
    def launch(self,mode,targets=None):
        ...
        def execute(job,rooms):
            ...
            def recorded(room,result,reason=None):
                results[room]=result
                try:self.history.record(ident,room,result,reason=reason)
            ...
            try:
                ...
                for room in list(rooms):
                    if room in DAILY_TASKS and ledger.done(ident,room):
                        recorded(room,'skipped')
                        log(TASK_LABELS[room]+': 오늘 완료됨 / KST 00:00 이후 다시 실행')
