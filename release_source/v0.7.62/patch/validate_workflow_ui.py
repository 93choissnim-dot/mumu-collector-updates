...
def check(app,output):
    ...
    unmaps=[]
    token=app.resume_bar.bind('<Unmap>',lambda event:unmaps.append(event.widget),add='+')
    for _ in range(20):app.sync_run_actions(False);root.update()
    assert not unmaps,'Stable footer was hidden during polling'
    app.resume_bar.unbind('<Unmap>',token)
    assert app.watch_switch.winfo_ismapped()
    assert app.watch_switch.winfo_rooty()+app.watch_switch.winfo_height()<=root.winfo_rooty()+root.winfo_height()
    enabled=app.game_watch.enabled
    with patch.object(app,'save'):
        app.watch_switch.toggle();root.update()
        assert app.game_watch.enabled!=enabled
        app.watch_switch.toggle();root.update()
        assert app.game_watch.enabled==enabled
