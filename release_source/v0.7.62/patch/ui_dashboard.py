...
from ui_layout import fit_window,preferred_size
from ui_state import selected_tasks as selected_tasks_for_layout
...
class Dashboard:
    def build(self):
        ...
        fit_window(self.root,preferred_size(self.config))
        ...
        identity=ctk.CTkFrame(self.toolbar,fg_color='transparent');self.identity_panel=identity
        identity.grid(row=0,column=0,sticky='ew',padx=(24,10),pady=(10,10));identity.grid_columnconfigure(1,weight=1)
        label(identity,'',image=icon('brand',color='#C9A660',size=36),width=38).grid(row=0,column=0,rowspan=2,padx=(0,12))
        heading(identity,'창키 도우미',size=23,color=CREAM,anchor='w').grid(row=0,column=1,sticky='w')
        identity.grid_columnconfigure(2,weight=0)
        ...
        actions=ctk.CTkFrame(self.toolbar,fg_color='transparent');self.toolbar_actions=actions;actions.grid(row=0,column=1,padx=(0,20))
        self.fleet_button=button(actions,'작업 설정',self.fleet_dialog,width=76,height=34,font=font(12))
        self.fleet_button.pack(side='left',padx=3)
        self.connect_button=button(actions,'뮤뮤 연결',self.connect,width=80,height=34,font=font(12))
        self.connect_button.pack(side='left',padx=3);self.controls.append(self.connect_button)
        self.update_button=button(actions,'업데이트',self.updates_dialog,width=68,height=34,font=font(11),fg_color='transparent')
        self.update_button.configure(text_color=CREAM,hover_color=ACCENT)
        self.update_button.pack(side='left',padx=3)
        self.more_button=button(actions,'더보기',self.open_more,width=56,height=34,font=font(11),fg_color='transparent')
        ...
        self.roster_total=label(roster_head,'',size=12,color=MUTED);self.roster_total.grid(row=0,column=1)
        ...
        label(tools,'뮤뮤 이름 검색',size=12,color=MUTED,anchor='w').grid(row=0,column=0,sticky='w',pady=(0,4))
        ...
        label(roster,'체크한 뮤뮤만 실행합니다.',size=12,color=MUTED).grid(row=3,column=0,sticky='w',padx=16,pady=(2,12))
        ...
        for col,title in enumerate(['현재 상태','다음 수령','수령 간격']):
            ...
            title_label=label(box,title,size=12,color=MUTED);title_label.pack(anchor='w');self.summary_titles.append(title_label)
        ...
        self.attention_label=label(section,'',size=11,color=GOLD,anchor='w',justify='left')
        ...
        for row_index,(key,title) in enumerate(TASK_LABELS.items()):
            ...
            stamp=label(row,'',size=12,color=MUTED,width=112,anchor='e');stamp.grid(row=0,column=3,sticky='e',padx=(0,12))
        ...
        self.log_summary=label(self.log_panel,'실행 기록이 여기에 표시됩니다.',size=12,color=MUTED,anchor='w')
        ...
        self.preview_stamp=label(detail,'최근 캡처 / 클릭하면 확대',size=12,color=MUTED)
        self.detail_checked=label(detail,'',size=12,color=MUTED)
        self.detail_stats=label(detail,'',size=12,color=MUTED,anchor='w')
        ...
        footer.pack(fill='x',padx=20,pady=10);footer.grid_columnconfigure(0,weight=1)
        ...
        label(info,'  이번 실행 수행 확인 ',size=12,color=MUTED).pack(side='left');self.count_label=label(info,'0건',size=11,color=MINT);self.count_label.pack(side='left')
        self.status_label=label(footer,textvariable=self.status,size=12,color=MUTED,anchor='w',wraplength=700,justify='left')
        self.status_label.grid(row=1,column=0,sticky='w',pady=(3,0))
        self.shortcut_hint=label(footer,'중지 키: '+self.stop_hotkey.value,size=12,color=MUTED,anchor='w')
        self.shortcut_hint.grid(row=2,column=0,sticky='w',pady=(3,0))
        self.once_button=button(footer,'한 번 실행',lambda:self.launch('once'),width=112,height=40)
        self.start_button=button(footer,'일반 자동 실행',lambda:self.launch('repeat'),primary=True,width=140,height=40)
        self.build_workflow()
        self.build_game_watch()
        ...
        self.controls.extend([self.once_button,self.start_button])
    ...
    def apply_layout(self):
        ...
        compact=width<1100;short=height<580;narrow_header=False;multi=len(self.players)>1;empty=not self.players;busy=self.busy()
        signature=(compact,short,narrow_header,multi,empty,self.compact_details,self.roster_collapsed,scale,self.main_panel._get_widget_scaling(),busy)
        self.status_label.configure(wraplength=max(180,int(width-60)))
        ...
        self.identity_panel.grid_configure(row=0,column=0,columnspan=1,padx=(12,4),pady=(8,8))
        self.toolbar_actions.grid_configure(row=1 if narrow_header else 0,column=0 if narrow_header else 1,columnspan=2 if narrow_header else 1,sticky='e',padx=(4,10),pady=0)
        self.toolbar.grid_configure(pady=0)
        self.main_panel.grid_configure(padx=12 if compact else 24,pady=8 if compact else 16)
        ...
        self.detail_summary_panel.grid_configure(pady=(8,6) if short else (12,10))
        for value in self.detail_summary:value.configure(font=font(15 if short else 21,True))
        self.detail_actions.grid_configure(pady=(5,6) if short else (8,10))
    ...
    def sync_run_actions(self,busy=None):
        ...
        for widget in (self.once_button,self.start_button,self.pause_button,self.stop_button,self.scope_selector):widget.grid_forget()
        self.footer.grid_columnconfigure(0,weight=1)
        self.run_info.grid_configure(row=0,column=0,columnspan=3,sticky='w')
        self.status_label.grid_configure(row=1,column=0,columnspan=3,sticky='w')
        self.shortcut_hint.grid_forget()
        first,second=(self.pause_button,self.stop_button) if busy else (self.once_button,self.start_button)
        if busy:self.shortcut_hint.grid(row=3,column=0,sticky='w',pady=(8,0))
        else:self.scope_selector.grid(row=3,column=0,sticky='w',padx=(0,8),pady=(8,0))
        first.grid(row=3,column=1,padx=(0,8),pady=(8,0))
        second.grid(row=3,column=2,pady=(8,0))
    ...
    def show_detail_tab(self,value):
        ...
        if value=='수령 결과':
            if len(selected_tasks_for_layout(self.players.get(self.view_id,{})))<len(REGULAR_LABELS):self.disabled_tasks_button.grid(row=0,column=1,sticky='w',padx=8)
    ...
    def layout_task_cards(self):
        ...
        entries=dict(self.display_entries(self.view_id))
        ...
        narrow=self.detail_content.winfo_width()/self.detail_content._get_widget_scaling()<540
    ...
    def stop_run(self):
        if hasattr(self,'game_watch'):self.game_watch.interrupt()
    ...
    def guide_dialog(self):
        ...
        for title,text in [('01  뮤뮤 연결','게임을 실행하고 뮤뮤 연결을 누릅니다. 목록의 이름을 누르면 해당 화면과 기록을 볼 수 있습니다.'),('02  뮤뮤별 작업 설정','각 뮤뮤의 설정에서 수령할 작업과 간격을 선택합니다. 수련은 재료를 사용하는 작업입니다.'),('03  실행과 제어','체크된 뮤뮤만 순서대로 실행합니다. 하단에서 일반, 일일 또는 전체 범위를 고르고 한 번 실행을 누릅니다. 자동 실행은 일반 작업만 반복합니다.'),('04  중지와 이어하기','일시중지 후 재시작하면 이어서 진행합니다. 종료 후에는 하단의 미완료 이어하기에서 대상과 보류 이유를 확인하세요. 같은 계정의 당일 기록만 이어갑니다.')]:
            label(win,title,size=13,bold=True).pack(anchor='w',padx=25,pady=(9,3))
            label(win,text,size=12,color=MUTED,wraplength=490,justify='left').pack(anchor='w',padx=25)
