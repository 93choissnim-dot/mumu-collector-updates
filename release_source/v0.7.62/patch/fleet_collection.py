...
class FleetCollection:
    def launch(self,mode,targets=None):
        ...
        packages={r.get('instance_id'):r.get('package') for r in self.device_reports.values()}
        watch=getattr(self,'game_watch',None)
        watch_jobs=self.watch_jobs() if watch else []
        watch_prepare=self.watch_prepare(executable) if watch else None
        ...
        def execute(job,rooms):
            ...
            try:
                ...
                known_package=packages.get(ident) or job.get('game_package')
                if known_package in GAME_PACKAGES:device.package=known_package
                ...
                if recovery is not None and watch is not None:
                    from game_watch import WatchedRecovery
                    def prepare_recovery(control):
                        recovery_device=AdbDevice(Adb(executable,control),serial,log=log,capture_mode=device.capture_mode)
                        recovery_device.package=device.package
                        return GameRecovery(recovery_device,vision,control,verify,log,lambda text:emit('fleet_progress',(ident,text)))
                    recovery=WatchedRecovery(watch,ident,prepare_recovery,self.stop)
        ...
        def operation():
            ...
            try:run_fleet(jobs,mode=='repeat',self.stop,self.update_pending,execute_tracked,emit,
                idle=(lambda:watch.scan(watch_jobs,watch_prepare,self.stop)) if watch else None)
