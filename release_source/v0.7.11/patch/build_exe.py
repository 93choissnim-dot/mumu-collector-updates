...
def build():
    ...
    feed={'app_id':'ChankiHelperExe','protocol':1,'version':VERSION,
        ...
        'sha256':sha(archive),'size':archive.stat().st_size,'notes':'중지 키 감지와 설정 저장 개선, 일괄 적용, 실패 재시도, 수령 기록, 절전 자동 복귀와 전체 선택 추가'}
...
def smoke(exe,result):
    ...
    assert report['ok'] and report['frozen'] and report['version']==VERSION and report['bits']==64 and report['full_ui_checks'] and report['game_theme_ui'] and report['improvements_ui'],report
