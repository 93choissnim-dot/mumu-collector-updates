...
from ui_theme import BG,PANEL,INSET,LINE,TEXT,MUTED,GOLD,RED,ACCENT,ACCENT_HOVER,CREAM,HEADER,HOVER,font,label,heading,button,panel,icon
...
class PlayerSettings:
    ...
    def select(self,ident):
        ...
        for row,title,keys in [(2,'자원 시설',list(LABELS)),(4,'추가 작업',[key for key in TASK_LABELS if key not in LABELS])]:
            title_row=ctk.CTkFrame(self.body,fg_color='transparent');title_row.grid(row=row,column=0,sticky='ew',padx=17,pady=(0,7));title_row.grid_columnconfigure(0,weight=1)
            label(title_row,title,size=13,bold=True).grid(row=0,column=0,sticky='w')
            if row==2:
            ...
            for i,key in enumerate(keys):
                tile=panel(group,fg_color=INSET);tile.grid(row=i//columns,column=i%columns,sticky='nsew',padx=4,pady=4)
                check=ctk.CTkCheckBox(tile,text=TASK_LABELS[key],variable=self.tasks[key],font=font(12),fg_color=ACCENT,border_color=MUTED,checkbox_width=19,checkbox_height=19)
                check.pack(anchor='w',padx=12,pady=(12,5));self.controls.append(check);self.task_checks[key]=check
                label(tile,TASK_HELP[key],size=10,color=GOLD if key=='training' else MUTED,wraplength=100 if row==2 else 150,justify='left').pack(anchor='w',padx=12,pady=(0,11))
        interval=panel(self.body,fg_color=INSET);interval.grid(row=6,column=0,sticky='ew',padx=16,pady=(0,10));interval.grid_columnconfigure(4,weight=1)
        ...
        presets=ctk.CTkOptionMenu(interval,values=['간격 선택','15분','30분','60분','120분'],width=102,height=30,font=font(11),fg_color=ACCENT,button_color=ACCENT,button_hover_color=ACCENT_HOVER,text_color=CREAM,command=lambda value:self.minutes.set(value[:-1]) if value!='간격 선택' else None)
    ...
    def bulk_dialog(self):
        ...
        choices={}
        for col,(field,title) in enumerate([('selected','시설과 작업'),('minutes','수령 간격')]):
        ...
        if self.draft[source]['selected'].get('training'):
            label(fields,'시설과 작업을 복사하면 재료를 사용하는 수련도 포함됩니다.',size=10,color=GOLD,wraplength=width-55,justify='left').grid(row=1,column=0,columnspan=3,sticky='w',pady=(8,0))
