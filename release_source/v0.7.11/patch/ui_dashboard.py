...
class Dashboard:
    def build(self):
        ...
        self.brand_subtitle=label(banner,'│   창세기전 키우기 수령 도우미',size=11,color='#DAD1BE');self.brand_subtitle.pack(side='left',padx=15)
        ...
        self.side_brand=label(side,'◇  창키 도우미  ◇',size=10,color=GOLD);self.side_brand.grid(row=7,column=0,pady=(14,18))
        ...
        self.header_note=label(header,'뮤뮤별 진행 상황과 다음 수령을 확인하세요.',size=11,color=MUTED);self.header_note.grid(row=1,column=0,sticky='w')
        ...
        metrics=panel(main,height=48);self.metrics_panel=metrics;metrics.grid(row=1,column=0,sticky='ew',pady=(0,10));metrics.grid_propagate(False)
        ...
        roster=panel(body);self.roster_panel=roster;roster.grid(row=0,column=0,sticky='nsew',padx=(0,10));roster.grid_columnconfigure(0,weight=1);roster.grid_rowconfigure(1,weight=1)
        ...
        self.detail_stats=label(detail,'',size=10,color=GOLD,height=17,anchor='w',wraplength=267)
        self.detail_stats.grid(row=5,column=0,sticky='w',padx=16,pady=(3,0))
        actions=ctk.CTkFrame(detail,fg_color='transparent');actions.grid(row=6,column=0,sticky='ew',padx=13,pady=(5,10));actions.grid_columnconfigure(0,weight=1)
        self.history_button=button(actions,'수령 기록',self.history_dialog,width=90,height=29,font=font(10));self.history_button.grid(row=0,column=0,sticky='w')
        self.inspect_button=button(actions,'화면 확인',self.inspect_selected,primary=True,width=92,height=29,font=font(10))
        self.inspect_button.grid(row=0,column=1,sticky='e');self.controls.append(self.inspect_button)
    ...
    def apply_layout(self):
        ...
        if compact:
            ...
            self.workspace_body.grid_columnconfigure(0,weight=1,minsize=470)
            self.workspace_body.grid_columnconfigure(1,weight=0,minsize=300)
            self.roster_panel.grid(row=0,column=0,columnspan=1,sticky='nsew',padx=(0,10))
