...
from ui_state import selected_tasks,task_summary,short_time,retry_tasks
...
class HistoryUI:
    def history_entries(self,ident):
        ...
        return {task:self.history.get(ident,task) or self.history.get(p.get('serial',''),task) for task in TASK_LABELS}
    ...
    def history_dialog(self,ident=None,issues_only=False):
        ...
        heading(win,p.get('name','뮤뮤')+' / 수령 기록',size=21,wraplength=width-42,justify='left').grid(row=0,column=0,sticky='w',padx=20,pady=(18,4))
        ...
        retry=button(footer,'실패 작업만 재시도',lambda:self.retry_failed(ident),primary=True,width=158)
        ...
        def render():
            ...
            entries=self.history_entries(ident);day=datetime.now().astimezone().date().isoformat()
            stats=self.history.stats(ident,alternate=p.get('serial',''))
            summary.configure(text=f"오늘 완료 {stats['today']}건   /   마지막 성공 {short_time(stats['last_success'])}")
            ...
            for index,(task,entry) in enumerate(rows):
                ...
                chosen=task in selected_tasks(self.players[ident])
                ...
                snapshot=failure_snapshot(DATA,ident,task,entry.get('checked_at'))
        ...
        render();self.history_render=render;self.history_mode=mode;self.history_retry_button=retry
