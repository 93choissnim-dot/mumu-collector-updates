...
import time
from daily_state import DailySchedule
from collector import Halt
from run_control import RunControl, checkpoint


def run_fleet(jobs, repeat, stop, update_pending, execute, event, clock=time.monotonic):
    if isinstance(stop, RunControl):
        clock = stop.clock
    schedules={j['id']:DailySchedule(j['rooms'],j['minutes']*60,clock) for j in jobs}
    ...
    while unfinished and not stop.is_set() and not update_pending.is_set():
        ...
        if due>clock():
            event('fleet_wait',due)
            if stop.wait(min(.25,max(0,due-clock()))):break
            continue
        ...
        unresolved=any(results.get(r) not in {'collected','skipped'} for r in rooms)
