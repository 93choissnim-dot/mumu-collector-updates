...
class DailyActions:
    ...
    def daily_tap(self,screen,key=None,point=None):
        ...
        self.device.click(point);self.pause(.65)
    def daily_main(self):
    ...
    def daily_open(self,key,states):
        main=self.daily_main();self.daily_tap(main,'open_'+key);return self.daily_wait(states)
    ...
    def collect_daily(self,task):
        ...
        self.daily_task=task;self.daily_day=korea_day();self.daily_reward_seen=False
        if self.daily_done('_complete'):return self.record_task(task,'skipped')
        ...
        return self.record_task(task,'collected')
    def daily_pass(self):
        pages={'daily_pass_'+k for k in ('ad','keys','gear')}
        self.daily_open('pass',pages)
        for key,y in [('ad',90),('keys',148),('gear',204)]:
            if self.daily_done(key):continue
            self.progress('패스 수령: '+{'ad':'광고 제거','keys':'던전 멤버십','gear':'장비 멤버십'}[key])
            ...
            if key in SWEEP_DUNGEONS:
                ...
                for _ in range(16):
                    ...
                    if self.daily_has(screen,'sweep_free_1') and self.daily_has(screen,'sweep_count_0'):
                        self.daily_tap(screen,point=(480,382))
                ...
                for _ in range(16):
                    ...
                    if zero and self.daily_has(screen,'d_free_1'):
                        self.daily_tap(screen,point=(744,440))
    ...
    def daily_guild(self):
        ...
        if not self.daily_done('attendance'):
            if not self.daily_has(screen,'guild_attended'):
                self.daily_tap(screen,point=(78,478));screen=self.daily_wait({'daily_guild_menu'})
        ...
        if not self.daily_done('donation'):
            ...
            for _ in range(5):
                ...
                if screen.state=='daily_guild_menu':
                    if self.daily_has(screen,'guild_donated'):self.daily_mark('donation');break
            ...
            for _ in range(5):
                ...
                if self.daily_has(screen,'guild_count_0'):
                    if not self.daily_has(screen,'guild_loot_zero'):
                        ...
                        self.daily_tap(screen,'guild_loot_claim');screen=self.daily_wait({'daily_guild_dungeon'})
                    if self.daily_has(screen,'guild_loot_zero'):self.daily_mark('dungeon');break
