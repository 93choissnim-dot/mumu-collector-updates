...
class RewardRecoveryTests(unittest.TestCase):
    ...
    def test_training_old_pending_uses_current_button_without_level_gate(self):
        for empty in (False,True):
            d,c=self.extra('training',empty=empty);c.action_state.reserve('training')
            self.assertEqual(c.claim_extra('training',d.screen()),'collected')
            self.assertEqual(d.claims,0 if empty else 1)
            self.assertFalse(c.action_state.pending('training'))
    ...
    def test_pause_during_reservation_cannot_reuse_manual_permission(self):
        ...
        def interrupted(task,slot='claim',**kwargs):
            reserve(task,slot,**kwargs);c.stop.pause();c.stop.resume()
