...
def health_check(output,full_ui=True):
    ...
    suite=unittest.TestSuite(unittest.defaultTestLoader.loadTestsFromTestCase(case)
        for case in (NativeTrainingRetryTests,ExplicitDungeonRetryTests,DiagnosticRecognitionTests,DiagnosticFlowTests,NativePassTests,TrainingTests,CombatStartTests,NativeDungeonTests,NativeRaidTests,DungeonConfirmationTests,CompletionRecheckTests,CompletionReviewUITests,CompletionReviewFleetTests,HistoryCountTests,PendingHistoryTests,RaidRetryTests,InputProvenanceTests,InputEvidenceTests,NativeAutumnTests,NativeNavigationTests,RetryResolutionTests,HistoryLifecycleTests,RewardRecoveryTests,UpdateDiagnosticsTests,
    ...
    output.write_text(json.dumps({'ok':True,'version':VERSION,'frozen':bool(getattr(sys,'frozen',False)),
        ...
        'daily_native_color':True,'worldboss_recovery':True,'daily_step_recovery':True,'diagnostic_trace':True,'autumn_rewards':True,'training_native_retry':True,'diagnostic_recovery':True,'training_repeatable_donation':True,'native_paid_pass':True,'training_input_evidence':True,'dungeon_native_entry':True,'combat_start_confirmation':True,'raid_native_entry':True,'dungeon_confirmation':True,'dungeon_completion_review':True,'raid_repeat_recovery':True,'pending_history_counts':True,'input_provenance':True,'input_evidence':True,'autumn_native_zero':True,'native_navigation':True,'input_safety':True,'pass_icon_identity':True,'efficiency_safety':True,
