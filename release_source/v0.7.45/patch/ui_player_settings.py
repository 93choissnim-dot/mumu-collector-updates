...
class PlayerSettings:
    ...
    def capture(self):
        ...
        self.draft[self.active]={'enabled':self.enabled.get(),'minutes':self.minutes.get(),
                                'selected':{k:v.get() for k,v in self.tasks.items()},'restore_sleep':True,'daily_profile':self.daily_profile.get()}
    ...
    def select(self,ident):
        ...
        label(self.body,'패스 · 던전 · 길드는 메인 화면의 일일 퀘스트 수령으로 실행합니다.',size=10,color=MUTED).grid(row=5,column=0,sticky='w',padx=18,pady=(0,10))
