...
class DailyVision:
    def __init__(self,assets):
        ...
        atlases={'combat_controls':combat,'guild_confirm':guild_confirm}
        ...
        # Enabled and disabled relic controls have the same label, with slightly
        # different glyph rasterization. Share label shapes only; each state
        # retains its own strict color/contrast reference and competition.
    ...
    def exposure(self,located):
        ...
        for names in EXPOSURE_ANCHORS:
            # The fight label is page context, not a calibration reference.
            # Native downsampling can soften it while the ranking anchor still
            # satisfies the stricter exposure threshold.
            if any(n not in located or located[n][0] < (
                    self.specs[n]['threshold'] if names==('guild_dungeon_rank','guild_fight') and n=='guild_fight'
