...
def main(exe,archive,previous=None):
    ...
    with tempfile.TemporaryDirectory(prefix='Chanki Windows 한글 ') as folder:
        root=Path(folder);target=root/'창키 도우미.exe';shutil.copy2(previous or exe,target)
    ...
    report={'ok':True,'real_exe_update':True,'restart':True,'rollback':True}
    if previous:report['previous_release_helper']=True
    (exe.parent/('windows-previous-validation.json' if previous else 'windows-validation.json')).write_text(json.dumps(report),encoding='utf-8')
