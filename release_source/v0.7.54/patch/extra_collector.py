...
class ExtraCollector(AutumnActions,DailyActions,WorldBossActions,Collector):
    ...
    def record_task(self,task,result):
        ...
        if (task in {'autumn','training'} and result in {'deferred','attempted'}
    ...
    def claim_training(self,screen,*,record=True):
        ...
        def finish(result):
            ...
            if not record and value=='deferred' and self.on_issue:
                self.on_issue(task,'수련: 레벨 상승 미확인 / 중복 입력 보류')
            return value
        pending=self.action_state.pending(task,slot)
        has_pending=pending is not None
        resumed_count=getattr(self,'resume_counts',{}).pop(task,0)
        self.claim_counts[task]=max(int(has_pending),resumed_count)
        ...
        deadline=self.now()+15;previous=None;stable=0
        while self.now()<deadline:
            # Always capture: cached page confirmations are not independent level observations.
            screen=self.screen()
            ...
            level=read_training_level(self.last_image) if screen.state=='training' else None
            key=(active,empty,level) if active!=empty else None
            stable=stable+1 if key is not None and key==previous else int(key is not None)
            previous=key
            if stable>=2:
                if attempts:
                    if level is not None and level==baseline+1:
                        self.action_state.confirm(task,slot,source='training_level_increased',
                                                  training_confirmation={'before':baseline,'after':level,'observations':stable})
                        return finish('collected')
                elif empty:return finish('skipped')
                elif level is not None:
                    baseline=level
                    self.progress(EXTRA_LABELS[task]+': 실행 / 레벨 상승 확인 전 중복 입력 보류')
                    self.trace.step='claim'
                    def reserve():
                        # verified_tap has captured again; don't save a stale baseline.
                        if read_training_level(self.last_image)!=baseline:
                            raise ScreenChanged('수련 레벨이 바뀌어 입력을 보류합니다.')
                        self.action_state.reserve(task,slot,evidence={'training_level':baseline})
                    ...
                    attempts=True;self.trace.step='verify';previous=None;stable=0
