"""Restrained ivory and forest surfaces with native scalable controls."""
import math
import os
...
def button(parent,text,command,primary=False,**kwargs):
    options=dict(height=38,corner_radius=8,border_spacing=9,font=font(13,primary),fg_color=ACCENT if primary else INSET,
                 hover_color=ACCENT_HOVER if primary else HOVER,text_color=CREAM if primary else TEXT,text_color_disabled='#88968E')
    options.update(kwargs)
    return GameButton(parent,text=text,command=command,**options)


def panel(parent,**kwargs):
    options=dict(fg_color=PANEL,corner_radius=12,border_width=1,border_color=RULE)
    options.update(kwargs)
