...
class HistoryUI:
    ...
    def history_dialog(self,ident=None,issues_only=False,task_filter=None):
        ...
        def render():
            ...
            summary.configure(text=f"오늘 완료 (KST) {stats['today']}건   /   마지막 성공 {short_time(stats['last_success'])}",text_color=GOLD)
            blocked=self.retry_block_reason()
            retry_hint.configure(text=blocked or '일일 작업은 완료한 단계를 유지하고 미완료 항목을 다시 확인합니다.')
            ...
            for index,(task,entry) in enumerate(rows):
                ...
                if task=='training' or task in DAILY_LABELS:
