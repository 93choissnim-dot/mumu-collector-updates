...
def pending_choices(data,scope,task):
    ...
    for step in DAILY_STEPS[task]:
        ...
        if detail.get('pending') or (step=='raid' and entry.get(step)=='started'):
...
def resolve_choice(data,scope,task,choice,outcome,*,confirmed=False):
    ...
    if task=='training':
        ...
        pending=state.get(task).get('pending',{});pending.pop('claim',None)
        state.update(task,pending=pending,blocked=False,failures=0,fingerprint='',manual_resolution=stamp)
