...
class BossDevice:
    ...
    def screen(self):
        ...
        matches={}
        if self.state=='menu' and not self.hidden_entry:matches['top_worldboss']=m('top_worldboss',(706,28))
...
class RouteTests(unittest.TestCase):
    def run_flow(self,notices,*,before_run=None,**options):
    ...
    def test_selector_is_not_reopened_after_final_planned_target(self):
        for keys in (['kraken'],['cerberus','void'],[b[0] for b in BOSSES]):
            for back in (False,True):
                d,c,r=self.run_flow(keys,back_to_select=back)
                opens=sum(state=='menu' and point==(706,28) for state,point in d.actions)
                self.assertEqual(opens,1 if back else len(keys))
                self.assertEqual(d.entered,keys);self.assertEqual(d.state,'menu')
    def change_after_claim(self,change):
        def setup(d,c):
            click=d.click
            def modified(point):
                claim=d.state=='boss_rank' and point==(843,114)
                click(point)
                if claim:change(d)
            d.click=modified
        return setup
    def test_new_notice_does_not_expand_initial_plan(self):
        setup=self.change_after_claim(lambda d:d.notices.add('kraken'))
        d,c,r=self.run_flow(['cerberus','void'],before_run=setup)
        self.assertEqual(d.entered,['cerberus','void']);self.assertEqual(d.claims['kraken'],0)
    def test_planned_target_that_becomes_preparing_or_loses_notice_is_skipped(self):
        for change in (lambda d:d.preparing.add('void'),lambda d:d.notices.discard('void')):
            d,c,r=self.run_flow(['cerberus','void'],before_run=self.change_after_claim(change))
            self.assertEqual(d.entered,['cerberus']);self.assertEqual(r,{'worldboss':'collected'})
            self.assertEqual(d.state,'menu')
    def test_pending_target_is_planned_without_notice_and_never_claimed_twice(self):
        def setup(d,c):
            c.action_state.reserve('worldboss','kraken');screen=d.screen
            def hidden_notice():
                s=screen();s.matches.pop('x_boss_notice_kraken',None);return s
            d.screen=hidden_notice
        d,c,r=self.run_flow(['kraken'],empty=['kraken'],before_run=setup)
        ...
        self.assertEqual(sum(state=='menu' for state,point in d.actions),1)
    def test_preparing_change_on_final_preclick_capture_blocks_entry(self):
        def setup(d,c):
            tap=c.tap
            def modified(screen,name=None,**kwargs):
                if name=='x_boss_card_kraken':d.preparing.add('kraken')
                return tap(screen,name,**kwargs)
            c.tap=modified
        d,c,r=self.run_flow(['kraken'],before_run=setup)
        self.assertEqual(d.entered,[]);self.assertEqual(sum(d.claims.values()),0)
    def test_missing_entry_icon_blocks_navigation_without_guessing(self):
        d,c,r=self.run_flow(['kraken'],hidden_entry=True)
        self.assertEqual(r,{'worldboss':'failed'});self.assertEqual(d.entered,[]);self.assertEqual(d.actions,[])
    ...
    def test_notice_with_inactive_reward_is_skipped_without_claim(self):
        ...
        self.assertEqual(d.entered,['kraken']);self.assertEqual(sum(d.claims.values()),0)
        self.assertEqual(r,{'worldboss':'skipped'})
    def test_each_boss_has_one_unconfirmed_claim_limit(self):
        d,c,r=self.run_flow([b[0] for b in BOSSES],lost=99,persistent=True)
        self.assertEqual(list(d.claims.values()),[1,1,1]);self.assertEqual(c.claim_counts['worldboss'],3)
        self.assertEqual(r,{'worldboss':'deferred'})
    def test_uncertain_boss_does_not_repeat_before_next_boss(self):
        d,c,r=self.run_flow(['cerberus','kraken'],lost=1)
        self.assertEqual(list(d.claims.values()),[1,1,0]);self.assertEqual(c.claim_counts['worldboss'],2)
