...
class VisionTests(unittest.TestCase):
    ...
    def test_free_counts_survive_resampling_without_changing_number(self):
        for prefix,context in [('d',('room_stone','d_close')),('sweep',('sweep_title','sweep_close'))]:
            for count in (0,1):
                im=self.native(self.frame(*context,prefix+'_free_'+str(count)))
                # Native capture and the reference recording rasterize glyphs
                # differently. Both zero and one must survive the same filter.
                im=cv2.GaussianBlur(im,(3,3),.5)
                _,m=self.vision.recognize(im)
                self.assertIn('daily_'+prefix+'_free_'+str(count),m)
                self.assertNotIn('daily_'+prefix+'_free_'+str(1-count),m)
    def test_missing_free_numerator_is_not_exhaustion(self):
        for count in (0,1):
            name='d_free_'+str(count);im=self.frame('room_stone','d_close',name)
            x,y,r,b=self.vision.specs[name]['box'];im[y:b,x:x+12]=110
            _,m=self.vision.recognize(im)
            self.assertNotIn('daily_d_free_0',m);self.assertNotIn('daily_d_free_1',m)
    def test_verified_numeric_variant_is_not_hidden_by_rejected_variant(self):
        # These public references denote the same 0/3 but have different
        # backgrounds. Force the best-shape variant to fail its color gate.
        v=DailyVision(Path(__file__).parent/'assets')
        im=self.frame('room_relic','d_close','relic_zero')
        v.reference_stats['rune_zero']=(10000,1,v.templates['rune_zero'][0].astype(float))
        _,m=v.recognize(im)
        self.assertIn('daily_d_count_0',m)
    def test_guild_battle_resampled_label_still_requires_guild_context(self):
        names=('guild_title','guild_tabs','guild_battle','guild_raid_open')
        im=cv2.GaussianBlur(self.native(self.frame(*names)),(3,3),.5)
        state,m=self.vision.recognize(im)
        self.assertEqual(state,'daily_guild_battle')
        self.assertIn('daily_guild_dungeon_open',m)
        x,y,r,b=self.vision.specs['guild_title']['box'];im[y:b,x:r]=0
        self.assertNotEqual(self.vision.recognize(im)[0],'daily_guild_battle')
...
class CompletedOutcomeTests(unittest.TestCase):
    ...
    def test_seven_empty_dungeons_finish_without_entry_sweep_or_free_key(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            c.log.assert_any_call('일일 던전: 장비 보급소 / 입장 횟수 없음')
