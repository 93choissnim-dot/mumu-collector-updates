...
class PendingHistoryTests(unittest.TestCase):
    ...
    def test_ticket_consuming_dungeon_pending_requires_resolution(self):
        ...
        for task,step in [('daily_guild','dungeon')]:
    ...
    def test_dungeon_retry_launches_without_discarding_old_request(self):
        a=self.a;a.launch=Mock();ledger=ManualQuestLedger(Path(self.tmp.name)/'daily_manual.json')
        ledger.checkpoint('vm','daily_dungeons','equipment','uncertain',pending='sweep_action')
        before=ledger.detail('vm','daily_dungeons','equipment')
        a.history.record('vm','daily_dungeons','deferred')
        with patch('retry_resolution.show_resolution') as resolve:a.retry_failed('vm',['daily_dungeons'])
        resolve.assert_not_called();a.launch.assert_called_once_with('retry',{'vm':['daily_dungeons']})
        self.assertEqual(ManualQuestLedger(ledger.path).detail('vm','daily_dungeons','equipment'),before)
