...
class DailyActions(DailyExecution):
    ...
    def daily_ready(self,states,keys,timeout=25):
        ...
        while self.now()<end:
            ...
            observed=(screen.state,active)
            count=count+1 if active and observed==last else int(bool(active));last=observed
            if count>=2:return screen
            self.pause(.25)
    ...
    def daily_tap(self,screen,key=None,point=None,*,work=False,before_input=None):
        ...
        if hasattr(self,'trace'):
            ...
            self.trace.event('input',action=key or 'navigation',point=list(point),before=fresh.state)
        # Capturing, reserving a paid action and recording evidence can cross midnight.
        self.daily_check_day()
    ...
    def daily_pass_tab(self,key,y,pages):
        ...
        for attempt in range(3):
            ...
            if self.daily_has(screen,'pass_'+key+'_done'):
                self.progress('패스 수령: '+{'ad':'광고 제거','keys':'던전 멤버십','gear':'장비 멤버십'}[key]+(' / 이미 완료' if attempt==0 else ' / 수령 완료'))
                self.daily_mark(key);return
            self.daily_tap(screen,'pass_'+key+'_active')
        raise Halt('패스 수령 완료를 확인하지 못했습니다.')
