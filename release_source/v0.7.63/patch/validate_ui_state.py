...
class UIStateTests(unittest.TestCase):
    ...
    def test_display_prioritizes_issues_preserving_execution_order(self):
        import copy
        ...
        self.assertEqual(task_display_order(p,entries),['worldboss','ranking','wood','daily_pass','mine','farm'])
        self.assertEqual(selected_tasks(p),execution)
        self.assertEqual((p,entries),saved)
        expanded=task_display_order(p,entries,True)
        self.assertEqual([task for task in expanded if task in task_display_order(p,entries)],task_display_order(p,entries))
...
class ThemeLegibilityTests(unittest.TestCase):
    def test_normal_text_and_primary_button_contrast(self):
        from ui_theme import TEXT,MUTED,GOLD,PANEL,HEADER,ACCENT,TINTS,TONES
        def luminance(color):
            values=[int(color[i:i+2],16)/255 for i in (1,3,5)]
            linear=[v/12.92 if v<=.04045 else ((v+.055)/1.055)**2.4 for v in values]
            return sum(a*b for a,b in zip(linear,(.2126,.7152,.0722)))
        for foreground,background in [(TEXT,PANEL),(MUTED,PANEL),(GOLD,PANEL),(HEADER,ACCENT)]+[(TONES[k],TINTS[k]) for k in ('error','warning','active')]:
