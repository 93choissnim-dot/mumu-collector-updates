...
def health_check(output,full_ui=True):
    ...
    from validate_v063_recovery import NetworkDispatchTests,WatchStatusTests,DonationReturnTests,EntryHandoffTests
    ...
    suite=unittest.TestSuite(unittest.defaultTestLoader.loadTestsFromTestCase(case)
        for case in (EntryHandoffTests,NetworkDispatchTests,WatchStatusTests,DonationReturnTests,NetworkRecoveryTests,NetworkRecognitionTests,WatchTests,PackageRecoveryTests,FooterStabilityTests,SharedWatchTests,StartupRecoveryTests,InterruptedStartupTests,EmptyDailyRowsTests,PresentationBoundaryTests,LateFocusRaceTests,SessionTests,DailySelectionTests,GameRecoveryTests,RecoveryIntegrationTests,CompactReliabilityTests,ResumeTests,FreeOnlyTests,EmptyDashboardTests,ResumeInitializationTests,FinalReviewScopeTests,KeyProgressTests,NativeSweepBattleTests,GuildResumeTests,SweepRetryTests))
    ...
    try:
        ...
        if full_ui:
            ...
            checkpoint('compact_ui')
            from validate_compact_ui import check as check_compact
            check_compact(app,output)
            checkpoint('workflow_ui')
            from validate_workflow_ui import check as check_workflow
            check_workflow(app,output)
    ...
    output.write_text(json.dumps({'ok':True,'version':VERSION,'frozen':bool(getattr(sys,'frozen',False)),
        ...
        'recovery_handoff_v063':True,'network_dialog_recovery':True,'independent_game_watch':True,'startup_confirmation':True,'session_workflow':True,'game_exit_recovery':True,'workflow_ui':full_ui,'compact_reliability':True,'compact_ui':full_ui,'free_only_guild':True,'worldboss_plan':True,'update_retry':True,
