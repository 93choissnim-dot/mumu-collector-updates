...
class SweepTransitionTests(unittest.TestCase):
    ...
    def daily_image(self,*names):
        ...
        return im
    def run_route(self,*,returns_room=False,reward_delay=0,stuck=False,stop_after_key=False,key='equipment',wrong_room=False,native_ready=False,missed_free=0,free_expires=False,stop_before_retry=False):
        ...
        class Device:
            ...
            def capture(d):
                d.clock+=.08
                if d.claims and d.state=='free' and d.clock>=d.claim_at+1:
                    if free_expires:d.state='empty'
                    if stop_before_retry:d.stop.set()
                ...
                if native_ready:return test.daily_image('sweep_title','sweep_close','sweep_action')
            ...
            def click(d,point):
                ...
                if d.state=='room':d.opens+=1;d.state='free' if d.free else 'ready'
                    ...
                    d.claims+=1;d.claim_at=d.clock
                    if d.claims<=missed_free:return
                    d.free=False;d.state='room' if returns_room else 'ready'
    ...
    def test_unhandled_free_key_is_rechecked_and_claimed_before_sweep(self):
        ...
        self.assertIsNone(error);self.assertTrue(done)
        self.assertEqual((d.claims,d.sweeps),(2,1))
    def test_persistent_free_key_stops_with_bounded_free_only_retries(self):
        d,done,detail,error=self.run_route(key='summon',missed_free=99)
        self.assertIsNotNone(error);self.assertFalse(done)
        self.assertEqual((d.claims,d.sweeps),(3,0));self.assertLess(d.clock,50)
    def test_expired_free_offer_is_not_retried(self):
        d,done,detail,error=self.run_route(key='summon',missed_free=1,free_expires=True)
        self.assertIsNotNone(error);self.assertEqual((d.claims,d.sweeps),(1,0))
    def test_stop_while_waiting_for_free_key_prevents_retry(self):
        d,done,detail,error=self.run_route(key='summon',missed_free=1,stop_before_retry=True)
        self.assertIsNotNone(error);self.assertEqual((d.claims,d.sweeps),(1,0))
    ...
    def test_native_key_claim_to_enabled_sweep_and_reward(self):
        for key in ('equipment','summon'):
            with self.subTest(key=key):
                d,done,detail,error=self.run_route(key=key,native_ready=True,reward_delay=3)
                self.assertIsNone(error);self.assertTrue(done)
                self.assertEqual((d.claims,d.sweeps),(1,1))
