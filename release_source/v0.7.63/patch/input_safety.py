...
def verified_tap(collector, screen, name=None, point=None, *, before_input=None,
    ...
    if action=='leave_room' and not same_room:
        raise ScreenChanged('시설 표시가 바뀌어 뒤로가기를 보류합니다.')
    ...
    if name:
        ...
        if name.startswith('daily_pass_') and name.endswith('_active'):
            opposites.append(name[:-7]+'_purchase')
    ...
    collector.trace.event('input_dispatched',action=action or name or 'navigation',point=list(point),before=fresh.state)
    return fresh
