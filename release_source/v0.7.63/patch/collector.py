...
class Collector(ObservationWindow):
    ...
    def screen(self):
        ...
        if screen.state=='network_error':
            raise Halt('네트워크 연결 끊김 / 현재 작업을 보존하고 재접속을 확인합니다.')
        return screen
    ...
    def top_bar_tap(self,screen,key,*,validate=None):
        ...
        while True:
            ...
            if screen.state not in {'main','menu','unknown'}:
                raise Halt('상단 메뉴 확인 중 화면이 바뀌었습니다.')
            if screen.state in {'main','menu'} and marker in screen.matches:
                try:
                    fresh=self.tap(screen,marker,validate=validate,action='open_'+key)
                except ScreenChanged:
                    # No touch was dispatched. Re-observe the icon rather than
                    # treating one animation frame as a task failure.
                    self.trace.event('navigation_recheck',action='open_'+key)
                else:
                    self.post_input(fresh,marker,delay=.6)
                    return
            self.pause(.25);screen=self.screen()
    ...
    def wait_menu_open(self):
        """A retry needs continuous, recognized main controls after settling.

        An unknown/loading/modal frame never authorizes a repeat. The actual
        retry still uses click_match's fresh gate, so a late menu is not toggled.
        """
        deadline=self.now()+8
        previous=None;seen=0;since=self.now();overlays={}
        while self.now()<deadline:
            screen=self.screen()
            if self.dismiss_overlay(screen,overlays):
                previous=None;seen=0;self.pause(.25);continue
            key=screen.state if screen.state=='menu' or (
                screen.state=='main' and 'main_menu' in screen.matches) else None
            if key is not None and key==previous:seen+=1
            else:seen=int(key is not None);since=self.now()
            previous=key
            if key=='menu' and seen>=2:return screen
            if key=='main' and seen>=3 and self.now()-since>=2:return screen
            self.pause(.25)
        raise Halt('오른쪽 메뉴 전환 확인 시간 초과 / 현재 '+state_label(self.last_screen.state))
    ...
    def collect_room(self, room, screen=None):
        ...
        resumed = getattr(self, 'resume_counts', {}).pop(room, 0)
        pending = bool(self.action_state.pending(room))
        manual_retry = room in self.manual_retry_rooms
        # This permission belongs to one explicit run, including its reconnects.
        self.manual_retry_rooms.discard(room)
        self.claim_counts[room] = max(resumed, int(pending))
        ...
        if (manual_retry and pending and not resumed and room in ('farm','wood','mine')
                and screen.state == room+'_ready' and self.room_visible(screen,room)
                and self.claim_button(screen,room) == 'ready_'+room):
            # Only explicit free-resource retries may rearm. Keep the durable
            # pending record until a replacement is durably reserved; fresh input guards
            # still validate the page and button immediately before dispatch.
            self.claim_counts[room] = 0
            self.log(LABELS[room]+': 수동 재시도 / 현재 수령 버튼 확인 → 1회 재시도')
        attempted_before = self.claim_counts[room]
        ...
        for attempt in range(self.claim_counts[room]+1, 2):
            ...
            def reserve():
                self.action_state.reserve(room,repeat_authorized=bool(self.action_state.pending(room)))
                self.claim_counts[room]=attempt
        ...
        self.log(f"{LABELS[room]}: {detail} / 이번 실행 {self.claim_counts[room]-attempted_before}회 클릭")
