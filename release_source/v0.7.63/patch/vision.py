...
def state_label(state):
    labels = {"exit_dialog": "게임 종료 확인창", "sleep": "절전 화면", "main": "게임 화면", "menu": "오른쪽 메뉴",
              "equipment": "장비 비교창", "reward": "보상 표시", "offline_reward": "오프라인 보상", "unknown": "미인식", "network_error": "네트워크 연결 끊김",
              "ranking":"랭킹", "boss_select":"월드보스 선택", "boss":"월드보스",
              "boss_rank":"월드보스 랭킹", "relic":"유물", "excavation":"유물 발굴", "training":"수련",
              "event_menu":"이벤트 목록", "autumn":"가을맞이 이벤트"}
...
class Vision:
    def __init__(self, assets=None):
        ...
        self.autumn = AutumnVision(assets)
        from top_bar import TopBarVision
        self.top_bar=TopBarVision(assets,self.daily)
        self.templates = {}
        self.exit_variants = {}
        ...
        for name in self.specs:
            ...
            if name.startswith('exit_'):
                # Desktop references are slightly larger and shifted compared
                # with native ADB. Keep shape thresholds; vary only geometry.
                variants=[]
                for scale in (.96,.98,1.,1.02):
                    color=cv2.resize(im,None,fx=scale,fy=scale,interpolation=cv2.INTER_AREA)
                    shape=cv2.GaussianBlur(cv2.cvtColor(color,cv2.COLOR_BGR2GRAY),(3,3),.7)
                    variants.append((color,shape))
                self.exit_variants[name]=variants
        ...
        from navigation_vision import NavigationVision
        self.navigation=NavigationVision(self.templates,self.specs)
    ...
    def recognize(self, bgr):
        ...
        from network_notice import detect_network_notice
        notice=detect_network_notice(im)
        if notice is not None:
            return Screen('network_error',{'network_confirm':Match('network_confirm',1,0,notice.confirm)} if notice.confirm else {})
        gray = cv2.GaussianBlur(cv2.cvtColor(im, cv2.COLOR_BGR2GRAY), (3, 3), .7)
        matches, diagnostics = {}, {}
        for name, spec in self.specs.items():
            target = spec.get("alias", name)
            x1, y1, x2, y2 = spec["box"]
            pad = 28 if name in self.exit_variants else 7
            x0, y0 = max(0, x1-pad), max(0, y1-pad)
            roi = gray[y0:min(540, y2+pad), x0:min(960, x2+pad)]
            best=None
            for color,template in self.exit_variants.get(name,(self.templates[name],)):
                scores = cv2.matchTemplate(roi, template, cv2.TM_CCOEFF_NORMED)
                _, score, _, loc = cv2.minMaxLoc(scores)
                if best is None or score>best[0]:best=(score,loc,color,template)
            score,loc,color,template=best
        ...
        self.navigation.recognize(im,matches,diagnostics)
        ...
        diagnostics['_daily']=self.daily.diagnostics
        matches.update(daily_matches)
        if daily_state:
            return Screen(daily_state, matches, diagnostics)
        autumn_state,autumn_matches=self.autumn.recognize(im)
        diagnostics['_autumn']=self.autumn.diagnostics
        matches.update(autumn_matches)
        if autumn_state:return Screen(autumn_state,matches,diagnostics)
        top_matches,top_diagnostics=self.top_bar.recognize(im)
        matches.update(top_matches);diagnostics["_top_bar"]=top_diagnostics
