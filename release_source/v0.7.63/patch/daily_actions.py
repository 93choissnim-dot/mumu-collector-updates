...
class DailyActions(DailyExecution):
    ...
    def daily_wait(self,states,timeout=40):
        ...
        end=self.now()+timeout;overlays={};reward_count=0
        while self.now()<end:
            self.daily_check_day();screen=self.screen()
            reward_count=reward_count+1 if screen.state=='reward' and 'reward_close' in screen.matches else 0
            if reward_count>=2:self.daily_reward_seen=True
    ...
    def daily_ready(self,states,keys,timeout=25):
        ...
        def observed(s):
            ...
            counters=('sweep_count_0',) if s.state=='daily_sweep' else (
                ('d_count_0','d_count_zero1','d_count_zero2') if s.state.startswith('daily_room_') else ())
            evidence=tuple(k for k in counters if self.daily_has(s,k))
            return (s.state,active,evidence) if active else None
        ...
        while self.now()<end:
            ...
            count=count+1 if current and current==last else int(bool(current));last=current
            if count>=2:return screen
            self.pause(.25)
    ...
    def daily_try_tap(self,screen,key=None,point=None,**kwargs):
        ...
        try:self.daily_tap(screen,key,point,**kwargs)
            ...
            return False
        return True
    def arm_dungeon_resume(self):
        """Snapshot only requests inherited when a new user run starts."""
        if hasattr(self,'dungeon_resume_requests'):return
        self.dungeon_resume_requests={}
        for step in DUNGEONS:
            detail=self.daily_ledger.detail(self.daily_ident,'daily_dungeons',step,korea_day())
            if detail.get('pending'):
                self.dungeon_resume_requests[step]=(detail['pending'],detail.get('input_request'))

    def daily_can_resume_pending(self):
        if getattr(self,'daily_verification_only',False):return False
        if self.daily_task=='daily_dungeons':
            saved=getattr(self,'dungeon_resume_requests',{}).get(self.daily_step)
        elif (self.daily_task,self.daily_step)==('daily_guild','dungeon'):
            saved=getattr(self,'guild_resume_request',None)
        else:return False
        detail=self.daily_detail()
        return bool(detail.get('pending') and saved is not None and
                    saved==(detail['pending'],detail.get('input_request')))

    def arm_guild_resume(self):
        """Snapshot inherited guild fight only once, before the new run."""
        if hasattr(self,'guild_resume_request'):return
        detail=self.daily_ledger.detail(self.daily_ident,'daily_guild','dungeon',korea_day())
        self.guild_resume_request=((detail['pending'],detail.get('input_request'))
            if detail.get('pending')=='guild_fight' else None)

    def daily_retire_inherited_pending(self):
        if not self.daily_detail().get('pending'):return
        if not self.daily_can_resume_pending():raise OutcomeUnknown('이번 실행의 던전 입력 결과 미확인 / 재입력 보류')
        guild=self.daily_task=='daily_guild'
        self.daily_checkpoint('uncertain',pending=None,retry_authorized=True,
                              resolution_source='fresh_guild_run' if guild else 'fresh_dungeon_run',
                              reason='이전 결과 미확인 기록 보존 / 현재 횟수 재확인')
        if guild:self.guild_resume_request=None
        else:self.dungeon_resume_requests.pop(self.daily_step,None)

    def daily_committed_tap(self,screen,key,**kwargs):
        if self.daily_detail().get('pending') and not self.daily_can_resume_pending():
            raise OutcomeUnknown('이전 입력 결과 미확인 / 중복 실행 보류: '+key)
        def reserve():
            self.daily_retire_inherited_pending()
            self.daily_checkpoint('uncertain',pending=key)
        return self.daily_try_tap(screen,key,before_input=reserve,**kwargs)
    def daily_confirm_input(self,evidence=None):
        if self.daily_detail().get('pending'):
            fields={'resolution_source':evidence} if evidence else {}
            self.daily_checkpoint('running',pending=None,reason='',**fields)
    ...
    def daily_tap(self,screen,key=None,point=None,*,work=False,before_input=None,required=()):
        def reserve():
            ...
            if work or key in WORK_BUTTONS:
                self.daily_work_attempts=getattr(self,'daily_work_attempts',0)+1
        fresh=self.tap(screen,'daily_'+key if key else None,point,
                 before_input=reserve,validate=self.daily_check_day,action=key or 'navigation',
        ...
        if key=='sweep_action':
            # The sweep observer owns even a short reward popup.
            self.pause(.1)
        elif key in {'d_enter','guild_fight','raid_fight','battle_skip','raid_skip','battle_leave','donate_50'}:
            self.pause(.65)
    ...
    def daily_open(self,key,states):
        ...
        # The pass emblem stays usable with the right menu open. Closing that
        # menu can expose a chat drawer underneath it and lose a known route.
        main=self.daily_wait(DAILY_PAGES|{'main','menu'}) if key=='pass' else self.daily_main()
        if key=='pass' and main.state not in {'main','menu'}:main=self.daily_main()
        if main.state not in ({'main','menu'} if key=='pass' else {'main'}):raise Halt('일일 작업 진입 전 기본 화면 확인이 필요합니다.')
    ...
    def daily_combat(self,return_states,timeout=240,on_result=None,*,accept_clear=False):
        self.daily_combat_evidence=None
        # An unchanged room/loading screen must not consume the battle budget.
        end=self.now()+min(20,timeout);last=None;count=0;skipped=False;overlays={};result=False;entered=False
        while self.now()<end:
            self.daily_check_day();screen=self.screen()
            if self.dismiss_overlay(screen,overlays):
                last=None;count=0;self.pause(.25);continue
            ...
            if not entered and screen.state in {'daily_battle','daily_result','daily_clear'}:
                entered=True;end=self.now()+timeout
            if screen.state=='daily_result':
                self.daily_combat_evidence='daily_result'
                if not result and on_result:on_result()
                result=True
            elif screen.state=='daily_clear':
                if accept_clear:
                    self.daily_combat_evidence='daily_clear'
                    if not result and on_result:on_result()
                    result=True
                ...
                if not self.daily_has(screen,key):
    ...
    def collect_daily(self,task):
        ...
        review=task=='daily_dungeons' and (getattr(self,'daily_verification_only',False) or
            any(self.daily_needs_completion_review(k) for k in DUNGEONS))
        if review and self.daily_done('_complete'):
            self.daily_ledger.update(self.daily_ident,task,
                {'_complete':None,'_completion_before_review':'done'},self.daily_day)
        if self.daily_done('_complete') and not review:return self.record_task(task,DAILY_ALREADY[task])
    ...
    def daily_pass_tab(self,key,y,pages):
        ...
        while self.now()<deadline:
            screen=self.daily_ready({'daily_pass_'+key},['pass_'+key+'_done','pass_'+key+'_active','pass_'+key+'_purchase'],timeout=max(1,deadline-self.now()))
            if self.daily_has(screen,'pass_'+key+'_purchase'):
                self.progress('패스 수령 / 유료 항목 제외')
                self.daily_mark(key)
                self.daily_checkpoint('done',excluded=True,reason='유료 패스 구매 제외')
                return
    ...
    def daily_wait_sweep_action(self,page,timeout=25):
        """A free key can return to its room; reopen only that verified modal."""
        end=self.now()+timeout;reopens=0;free_retries=0;retry_at=self.now()+2
        while self.now()<end:
            screen=self.daily_ready({'daily_sweep',page},['sweep_action','d_sweep_open','sweep_free_1','sweep_free_0'],
                                    timeout=max(1,end-self.now()))
            if screen.state=='daily_sweep' and self.daily_has(screen,'sweep_action') and not self.daily_has(screen,'sweep_count_0'):return screen
            if (screen.state=='daily_sweep' and self.daily_has(screen,'sweep_free_1')
                    and self.daily_has(screen,'sweep_count_0') and not self.daily_has(screen,'sweep_free_0')
                    and self.now()>=retry_at and free_retries<2):
                # Free grant only: both free availability and zero entries are
                # revalidated before each retry; never replay an uncertain sweep.
                if self.daily_detail().get('pending'):raise OutcomeUnknown('소탕 결과 미확인 / 무료 열쇠 재입력 보류')
                if self.daily_try_tap(screen,point=(480,382),work=True,required=('sweep_free_1','sweep_count_0')):
                    free_retries+=1;retry_at=self.now()+2
            if screen.state==page and self.daily_has(screen,'d_sweep_open'):
                if reopens>=2:break
                if self.daily_try_tap(screen,'d_sweep_open'):reopens+=1
            self.forget_observations();self.pause(.25)
        raise Halt('무료 열쇠 수령 후 소탕 화면과 버튼 확인 시간 초과')

    def daily_wait_sweep_result(self,timeout=15):
        """Wait for delayed reward evidence without dispatching another sweep."""
        end=self.now()+timeout
        while self.now()<end:
            self.forget_observations()
            screen=self.daily_ready({'daily_sweep'},['sweep_action','sweep_free_0','sweep_free_1'],
                             timeout=max(1,end-self.now()))
            if self.daily_reward_seen:return screen
            self.pause(.3)
        raise OutcomeUnknown('소탕 보상 확인 시간 초과 / 같은 소탕 재입력 보류')
    ...
    def daily_dungeons(self):
        if any(self.daily_needs_completion_review(k) or (
                not self.daily_done(k) and self.daily_detail(k).get('status')!='blocked') for k in DUNGEONS):
            self.daily_open('dungeon',{'daily_dungeons'})
        for key in DUNGEONS:
            label='일일 던전: '+DUNGEON_NAMES[key]
            self.daily_run_step(key,label,lambda k=key:self.daily_dungeon(k))
        self.daily_main()
    def daily_dungeon(self,key,*,verification_only=False):
        verification_only=verification_only or getattr(self,'daily_verification_only',False)
        page='daily_room_'+key;screen=self.daily_find_dungeon(key)
        if verification_only and self.daily_detail().get('pending'):
            raise OutcomeUnknown('완료 기록 확인: 이전 입력 결과 미확인 / 입력 없이 보류')
        if key in SWEEP_DUNGEONS:
            zero_markers=('d_count_0','d_count_zero1','d_count_zero2')
            if any(self.daily_has(screen,k) for k in zero_markers) and self.daily_has(screen,'d_free_0'):
                screen=self.daily_ready({page},['d_sweep_open','d_free_0','d_free_1'])
                if any(self.daily_has(screen,k) for k in zero_markers) and self.daily_has(screen,'d_free_0'):
                    if self.daily_detail().get('pending') and not self.daily_can_resume_pending():
                        raise OutcomeUnknown('소탕 결과 미확인 / 중복 소탕 보류')
                    self.daily_retire_inherited_pending()
                    self.daily_checkpoint('done',values={key:'done'},pending=None,reason='',
                        completion_evidence={'kind':'no_entries','observations':2,'page':page,'revision':1})
                    self.daily_close_room(page);return
            self.daily_tap(screen,'d_sweep_open');screen=self.daily_wait({'daily_sweep'})
            self.daily_reward_seen=False;sweep_requested=False
            for _ in range(16):
                screen=self.daily_ready({'daily_sweep'},['sweep_action','sweep_free_0','sweep_free_1'])
                if self.daily_detail().get('pending'):
                    if sweep_requested:
                        if not self.daily_reward_seen:screen=self.daily_wait_sweep_result()
                        self.daily_confirm_input('reward')
                    elif not self.daily_can_resume_pending():raise OutcomeUnknown('소탕 결과 미확인 / 중복 소탕 보류')
                if self.daily_has(screen,'sweep_count_0') and self.daily_has(screen,'sweep_free_0'):
                    self.daily_retire_inherited_pending()
                    self.progress('일일 던전: '+DUNGEON_NAMES[key]+' / 입장 횟수 없음')
                    self.daily_checkpoint('done',values={key:'done'},pending=None,reason='',
                        completion_evidence={'kind':'no_entries','observations':2,'page':'daily_sweep','revision':1});break
                if verification_only:raise OutcomeUnknown('완료 기록 재확인: 남은 횟수 또는 무료 열쇠 있음 / 재시도 필요')
            ...
            for _ in range(16):
                ...
                if zero and self.daily_has(screen,'d_free_0'):
                    ...
                    self.progress('일일 던전: '+DUNGEON_NAMES[key]+' / 입장 횟수 없음')
                    self.daily_checkpoint('done',values={key:'done'},pending=None,reason='',
                        completion_evidence={'kind':'no_entries','observations':2,'page':page,'revision':1});break
                if verification_only:raise OutcomeUnknown('완료 기록 재확인: 남은 횟수 또는 무료 열쇠 있음 / 재시도 필요')
                if zero and self.daily_has(screen,'d_free_1'):
                    ...
                    if not self.daily_try_tap(screen,point=(744,440),work=True,required=('d_free_1',counter),before_input=self.daily_retire_inherited_pending):continue
                    self.daily_wait_marker({page},'d_enter')
                    # Key grant and entry are separate server/UI transitions.
                    # Discard observations from the grant before accepting entry.
                    self.pause(1.25);self.forget_observations();continue
                if self.daily_has(screen,'d_enter'):
                    if not self.daily_committed_tap(screen,'d_enter'):continue
                    screen,result=self.daily_combat({page},accept_clear=True,on_result=lambda:
                        self.daily_confirm_input(self.daily_combat_evidence))
                    if not result:raise OutcomeUnknown('던전 전투 결과 미확인 / 중복 입장 보류')
                    self.daily_confirm_input(getattr(self,'daily_combat_evidence',None))
    ...
    def daily_guild_donation(self):
        ...
        for attempt in range(5):
            ...
            if self.daily_has(screen,'donate_free'):self.daily_tap(screen,'donate_free')
                ...
                self.daily_mark('donation')
                self.daily_checkpoint('done',excluded=True,reason='유료 길드 기부 제외 / 루비 사용 안 함')
                self.progress('길드: 무료 기부 확인 / 유료 기부 제외')
                # A delayed free-donation reward may cover the close button.
                # Reobserve/dismiss that overlay; never donate again to navigate.
                for _ in range(4):
                    if self.daily_try_tap(screen,'donate_close'):
                        self.daily_wait({'daily_guild_menu','daily_guild_battle'})
                        return
                    screen=self.daily_wait({'daily_donate','daily_guild_menu','daily_guild_battle'})
                    if screen.state!='daily_donate':return
                raise Halt('길드 기부 보상 후 복귀 화면 확인 시간 초과')
            else:raise Halt('길드 기부: 무료 버튼 또는 유료 제외 상태를 확인하지 못했습니다.')
    ...
    def daily_guild_dungeon(self):
        ...
        for _ in range(12):
            ...
            if self.daily_detail().get('pending') and not self.daily_can_resume_pending():
                raise OutcomeUnknown('길드 던전 입장 결과 미확인 / 중복 입장 보류')
            if self.daily_has(screen,'guild_count_0'):
                ...
                if not self.daily_has(screen,'guild_loot_zero'):
                    if not (any(self.daily_has(screen,'guild_loot_'+str(n)) for n in (10,20,30)) or
                            self.daily_has(screen,'guild_loot_available')):
                        raise Halt('길드 전리품 개수 확인이 필요합니다.')
                    # The generic red-notification claim control supports
                    # arbitrary positive loot counts; completion still needs zero.
                    if self.daily_has(screen,'guild_loot_available'):
                        screen=self.daily_ready({'daily_guild_dungeon'},['guild_loot_available'])
                        self.daily_tap(screen,'guild_loot_claim',required=('guild_loot_available','guild_count_0'))
                    else:self.daily_tap(screen,'guild_loot_claim')
                    claimed=True;screen=self.daily_wait({'daily_guild_dungeon'})
            ...
            _,result=self.daily_combat({'daily_guild_dungeon'},accept_clear=True,on_result=lambda:
                self.daily_confirm_input(self.daily_combat_evidence))
            if not result:raise OutcomeUnknown('길드 던전 전투 결과 미확인 / 중복 입장 보류')
            self.daily_confirm_input(getattr(self,'daily_combat_evidence',None))
    ...
    def daily_guild_raid(self):
        ...
        # Repeating this raid is allowed. Only retire the old uncertainty once
        # the detail page and fight button pass the fresh pre-input guard.
        # The collector lives across pause/connection recovery. Keep its budget
        # here so re-entering collect_daily cannot dispatch a second fight.
        # A new user run creates a new collector and may retry the old pending.
        def reserve():
            attempts=getattr(self,'daily_raid_attempts',set())
            attempt=(self.daily_ident,self.daily_task)
            if attempt in attempts:
                raise OutcomeUnknown('이번 실행의 공방 약탈 입력 결과를 확인하지 못했습니다. 새로 재시도하면 전투 버튼을 다시 확인합니다.')
            self.daily_raid_attempts=attempts
            attempts.add(attempt)
            if status=='started' or self.daily_detail().get('pending'):
                self.daily_checkpoint('running',pending=None,retry_authorized=True,
                                      resolution_source='repeat_allowed_raid',reason='')
            self.daily_checkpoint('uncertain',pending='raid_fight',values={'raid':'started'})
        ...
        if not result:raise OutcomeUnknown('공방 약탈 결과를 확인하지 못했습니다. 재시도 시 전투 버튼을 다시 확인합니다.')
