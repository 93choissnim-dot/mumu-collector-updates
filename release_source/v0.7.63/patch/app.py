...
class App(GameWatchUI, FleetCollection, FleetUI, Dashboard, Updates, RunControls, RosterUI, WorkflowUI, HistoryUI):
    def __init__(self, root, autoconnect=True, tray=True):
        ...
        self.worker_generation=0;self.worker_pending=None
        ...
        self.init_game_watch(live=autoconnect)
        self.init_fleet()
        self.build()
    ...
    def busy(self):
        return getattr(self,'worker_pending',None) is not None or (self.worker is not None and self.worker.is_alive())
    ...
    def save(self):
        ...
        from ui_layout import window_size
        size=window_size(self.root)
        if size:self.config['window_size']=size
    ...
    def on_device_changed(self, value=None):
        ...
        if report is None:
            ...
            self.connection_badge.configure(text_color=HEADER_MUTED)
        elif serial in self.unavailable_devices:
            self.connection_text.set('연결을 다시 확인해 주세요.')
            self.connection_badge.configure(text_color=HEADER_GOLD)
            ...
            self.connection_badge.configure(text_color=HEADER_SUCCESS if known else HEADER_GOLD)
    ...
    def run_worker(self,operation,status,pausable=False):
        ...
        self.worker_generation=getattr(self,'worker_generation',0)+1
        generation=self.worker_generation;self.worker_pending=generation
        self.game_watch.interrupt()
        self.stop.clear();self.status.set(status)
        self.running_status=status;self.pause_allowed=pausable
        self.stop_hotkey.reset()
        self.failure=False;self._has_run_summary=False;self.finish_text="작업을 마쳤습니다."
        self.set_busy(True)
        def work():
            try:
                with self.device_lane:operation()
            except Halt as exc:
                self.log("중지: "+str(exc))
        ...
        try:self.worker.start()
            ...
            self.worker_pending=None;self.worker=None;self.set_busy(False)
            raise
        self.sync_stop_key()

    def sync_stop_key(self):
        if self.hotkey_listener:
            signature=(self.stop_hotkey.value,(self.busy() or self.game_watch.enabled) and not self.hotkey_capture and not self.closing)
            if signature!=getattr(self,'_hotkey_signature',None):
                self._hotkey_signature=signature;self.hotkey_listener.configure(*signature)

    def on_stop_key(self):
        # Native key thread: cancellation is immediate; all Tk changes are queued.
        if (self.busy() or self.game_watch.enabled) and not self.hotkey_capture and not self.closing:
            self.game_watch.enable(False)
    ...
    def poll(self):
        if os.name=="nt" and not self.hotkey_capture and self.hotkey_listener is None:
            if self.stop_hotkey.poll(lambda key:ctypes.windll.user32.GetAsyncKeyState(key)&0x8000) and (self.busy() or self.game_watch.enabled):
                self.on_stop_key()
        history_changed=False
        try:
            while True:
                kind,value=self.events.get_nowait()
                if kind=='done' and value!=getattr(self,'worker_pending',None):continue
                if kind in {'fleet_result','fleet_status','done','failed','cancelled','room_result'}:
                    history_changed=True
                if kind in {'fleet_status','fleet_result','fleet_progress','fleet_active','device_checked','connected',
                            'link_available','link_unavailable','done','failed','cancelled','hotkey_stop'}:
                    self.roster_dirty=True
                if kind.startswith("update_"):
                    self.handle_update_event(kind,value)
                elif kind=='watch_package':
                    ident,package=value
                    if ident in self.players and self.players[ident].get('game_package')!=package:
                        self.players[ident]['game_package']=package;self.save()
                elif kind=='game_watch':
                    self.watch_status.configure(text=value)
                    if '보류' in value or '재접속 완료' in value:self.log('게임 자동 복구 / '+value)
                elif kind=="tray_ready":self.tray_ready=True
                elif kind=='hotkey_stop':
                    if value==self.stop.generation:
                        self.watch_value.set(False);self.config['game_watch']=False;self.stop_run();self.save()
                ...
                elif kind=='fleet_active':
                    ident,task=value;self.fleet_states.setdefault(ident,{})['active_task']=task
                elif kind=='fleet_entry':
                    ident,task,entry=value;self.fleet_states.setdefault(ident,{}).setdefault('run_entries',{})[task]=entry
                elif kind=='fleet_summary':
                    self._has_run_summary=True
                    from ui_state import execution_summary
                    self.finish_text=execution_summary(value,self.stop.is_set())
                elif kind=="fleet_status":
                    ident,state=value;self.fleet_states.setdefault(ident,{}).update(state)
                    if state.get("status") in {"수령 중","재시도 중"}:
                        self.active_instance=ident
                    ...
                    self.worker_pending=None;self.active_instance=None
                    if hasattr(self,'refresh_workflow'):self.refresh_workflow(force=True)
                    self.sync_stop_key()
                    self.stop.resume()
                    ...
                    if not getattr(self,'_has_run_summary',False):self.finish_text="중지했습니다. 선택한 뮤뮤는 유지됩니다." if self.chosen_serial() else "중지했습니다."
                    ...
                    self.remember_game_packages(reports)
                    self.on_device_changed()
                    from ui_state import connection_summary
                    self.finish_text=connection_summary(reports)
                    if not self.device_options:
                        ...
                        self.connection_badge.configure(text_color=HEADER_ERROR)
                    ...
                    self.remember_game_packages({serial:report})
        ...
        history_state=(self.busy(),self.stop.paused,self.update_pending.is_set(),self.update_applying)
        if history_changed or history_state!=getattr(self,'history_control_state',None):
            self.history_control_state=history_state
            self.request_history_refresh()
        if self.stop.paused:
        ...
        self.status_label.configure(wraplength=max(220,int(self.root.winfo_width()/self.root._get_window_scaling())-(52 if self.compact_layout else 360)))
        self.poll_game_watch()
        self.sync_stop_key()
    ...
    def close(self):
        ...
        self.closing=True;self.game_watch.enable(False);self.stop.set()
