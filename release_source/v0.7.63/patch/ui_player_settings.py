...
from ui_theme import HEADER,BG,PANEL,INSET,LINE,RULE,TEXT,MUTED,GOLD,RED,ACCENT,ACCENT_HOVER,CREAM,HEADER,HOVER,font,label,heading,button,panel,icon
from ui_state import selected_tasks,validate_profile
from task_catalog import REGULAR_LABELS as TASK_LABELS
...
class PlayerSettings:
    def __init__(self,app,initial=None):
        ...
        self.draft={key:{'enabled':bool(p.get('enabled')),'minutes':format(p.get('minutes',60),'g') if isinstance(p.get('minutes',60),(int,float)) else str(p.get('minutes',60)),
                    ...
                    'daily_selected':{t:t in selected_daily(p) for t in DAILY_LABELS},'restore_sleep':True,'daily_profile':p.get('daily_profile','')} for key,p in app.players.items()}
        self.original=copy.deepcopy(self.draft)
        win=ctk.CTkToplevel(app.root);self.window=win;app.fleet_window=win
        win.title('작업 설정');win.configure(fg_color=BG);win.transient(app.root)
        ...
        heading(top,'작업 설정',size=23).grid(row=0,column=0,sticky='w')
        ...
        nav=ctk.CTkScrollableFrame(body,width=nav_width,fg_color=INSET,corner_radius=10,border_width=1,border_color=RULE,scrollbar_button_color=LINE)
        ...
        self.body=ctk.CTkScrollableFrame(body,fg_color=PANEL,corner_radius=10,border_width=1,border_color=RULE,scrollbar_button_color=LINE)
    ...
    def capture(self):
        ...
        self.draft[self.active]={'enabled':self.enabled.get(),'minutes':self.minutes.get(),
                                'selected':{k:v.get() for k,v in self.tasks.items()},'daily_selected':{k:v.get() for k,v in self.daily_tasks.items()},'restore_sleep':True,'daily_profile':self.daily_profile.get()}
    ...
    def select(self,ident):
        ...
        for key,b in self.profile_buttons.items():b.configure(fg_color=HEADER if key==ident else 'transparent',text_color=CREAM if key==ident else TEXT,hover_color=ACCENT_HOVER if key==ident else HOVER)
        self.enabled=tk.BooleanVar(value=p['enabled']);self.minutes=tk.StringVar(value=p['minutes'])
        self.daily_profile=tk.StringVar(value=p.get('daily_profile',''))
        self.tasks={key:tk.BooleanVar(value=p['selected'][key]) for key in TASK_LABELS}
        self.daily_tasks={key:tk.BooleanVar(value=key in selected_daily(p)) for key in DAILY_LABELS}
        ...
        self.interval_presets=ctk.CTkOptionMenu(interval,values=list(INTERVAL_PRESETS),width=112,height=32,font=font(11),fg_color=ACCENT,button_color=ACCENT,button_hover_color=ACCENT_HOVER,text_color=HEADER,command=self.choose_interval)
        ...
        for i,key in enumerate(TASK_LABELS):
            ...
            check.grid(row=i*2,column=0,sticky='w',padx=(14,8),pady=12);self.controls.append(check);self.task_checks[key]=check
            label(self.task_group,TASK_HELP[key],size=11,color=GOLD if key=='training' else MUTED,wraplength=170,justify='left',anchor='w').grid(row=i*2,column=1,sticky='w',padx=(0,14),pady=12)
        ...
        daily=panel(self.body,fg_color=INSET);daily.grid(row=5,column=0,sticky='ew',padx=16,pady=(0,12))
        label(daily,'일일 작업 / 무료 항목만 실행',size=13,bold=True).pack(anchor='w',padx=14,pady=(12,6))
        for key,title in DAILY_LABELS.items():
            check=ctk.CTkCheckBox(daily,text=title,variable=self.daily_tasks[key],font=font(12),fg_color=ACCENT,border_color=MUTED)
            check.pack(anchor='w',padx=14,pady=8);self.controls.append(check)
        label(daily,'하단에서 일일 작업 또는 전체를 선택해 실행합니다.\n자동 실행은 일반 작업만 반복합니다.',size=12,color=MUTED,wraplength=330,justify='left').pack(anchor='w',padx=14,pady=(4,12))
        identity=panel(self.body,fg_color=INSET);identity.grid(row=6,column=0,sticky='ew',padx=16,pady=(0,12));identity.grid_columnconfigure(0,weight=1)
        label(identity,'계정/캐릭터 구분 (선택)',size=12,bold=True).grid(row=0,column=0,sticky='w',padx=12,pady=(10,5))
