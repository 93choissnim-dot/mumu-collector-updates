...
class ManualQuestLedger(DailyLedger):
    ...
    def begin_run(self,ident,legacy_path=None,tasks=None):
        with self.lock:
            ...
            selected=set(DAILY_TASKS if tasks is None else tasks)
            fresh={k:copy.deepcopy(v) for k,v in prior.items() if k not in selected}
