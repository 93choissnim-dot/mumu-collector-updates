...
def profile_values(player):
    return {'enabled':bool(player.get('enabled')), 'minutes':float(player.get('minutes',60)),
            ...
            'daily_selected':{t:t in selected_daily(player) for t in DAILY_LABELS},
...
def merge_profiles(current, original, draft):
    ...
    for ident, values in draft.items():
        ...
        for task in DAILY_LABELS:
            change(result['daily_selected'],task,before['daily_selected'][task],after['daily_selected'][task],latest['daily_selected'][task])
