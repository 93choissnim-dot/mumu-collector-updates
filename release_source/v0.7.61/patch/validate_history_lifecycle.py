...
class HistoryLifecycleTests(unittest.TestCase):
    def setUp(self):
        ...
        a.config={};a.init_game_watch(live=False);a.watch_status=Widget()
    ...
    def test_new_worker_cancels_watch_then_waits_for_exclusive_device_lane(self):
        a=self.a;entered=threading.Event();release=threading.Event();executed=threading.Event()
        a.game_watch.control=RunControl();control=a.game_watch.control
        def watch():
            with a.device_lane:entered.set();release.wait(2)
        thread=threading.Thread(target=watch);thread.start();self.assertTrue(entered.wait(1))
        try:
            a.run_worker(executed.set,'inspect')
            self.assertTrue(control.is_set());self.assertFalse(executed.wait(.05))
        finally:release.set();thread.join(2)
        a.worker.join(2);self.assertTrue(executed.is_set())
    ...
    def test_pending_resolution_is_only_required_for_daily_resource_issues(self):
