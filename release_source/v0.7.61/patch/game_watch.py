...
class GameWatch:
    def __init__(self,report,clock=time.monotonic,package_report=None):
        ...
        self.startups={};self.packages={};self.package_report=package_report or (lambda ident,package:None)
        ...
        try:
            for job in jobs:
                ...
                try:
                    ...
                    if recovered:self.status(job.get('name',ident)+' / 재접속 완료, 수령 예약 유지')
                    else:self.status('감시 중 / 선택한 뮤뮤')
                except Exception as exc:
                    if not control.is_set() and not (parent is not None and (parent.is_set() or parent.paused)):
                        self.status(job.get('name',ident)+' / 복구 보류: '+str(exc))
