...
class History:
    ...
    def record(self, serial, room, result, now=None, reason=None, run_id=None, progress=None):
        ...
        with self.lock:
            ...
            versioned=previous.get('failure_count_version')==2
            # Old streaks included deferred results; their composition is unknowable.
            legacy=count('legacy_consecutive_issues') if versioned else count('consecutive_failures')
            failures=count('consecutive_failures') if versioned else 0
            holds=count('consecutive_holds') if versioned else 0
            entry = {"checked_at": stamp, "result": result, "run_id":run_id,
                           "collected_at": stamp if result == "collected" else previous.get("collected_at"),
                           'daily':daily,'failure_count_version':2,
                           'consecutive_failures':failures+1 if result in FAILURES else failures if result=='deferred' else 0,
                           'consecutive_holds':holds+1 if result=='deferred' else 0,
                           'reason':str(reason or (RESULTS[result] if result in ISSUES else ''))[:2000]}
            if isinstance(progress,dict):entry['progress']=copy.deepcopy(progress)
            if legacy:entry['legacy_consecutive_issues']=legacy
    ...
    def stats(self,serial,day=None,alternate=None):
        ...
        return {'today':total,'last_success':max(stamps).isoformat() if stamps else None,
                'failed_tasks':sum(entry.get('result') in FAILURES for entry in entries),
                'held_tasks':sum(entry.get('result')=='deferred' for entry in entries),
                'issue_tasks':sum(entry.get('result') in ISSUES for entry in entries)}
