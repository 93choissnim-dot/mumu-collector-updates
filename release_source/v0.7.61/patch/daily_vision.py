...
EXPOSURE_ANCHORS=(('guild_title','guild_tabs'),
    ...
    ('shop_confirm_title','shop_confirm_close'),
    ('guild_dungeon_rank_diagnostic','guild_fight_diagnostic'))+tuple(
...
def classify(names):
    ...
    if (has('battle_skip','battle_auto') or has('guild_battle_header','battle_skip') or
        has('treasure_battle_header','treasure_battle_hud') or 'raid_skip' in names):return 'daily_battle'
    return None
...
class DailyVision:
    def __init__(self,assets):
        ...
        native=json.loads((p/'diagnostic_controls.json').read_text(encoding='utf-8'))['templates']
        self.specs.update(native)
        # Native control shape supplements the old sweep rasterization. Its
        # original color reference still decides whether the action is enabled.
        native['sweep_action_diagnostic']={'box':[453,367,507,397], 'tile':[0,0,54,30],
            'atlas':'native_213124_controls', 'reference_only':True, 'threshold':.94}
        self.specs.update(native)
        self.specs.update({
            'treasure_battle_header':{'box':[36,22,116,43], 'tile':[0,34,80,55],
                'atlas':'native_213124_controls', 'threshold':.94, 'strict':True, 'blur':True},
            # The objective text overlays animated scenery and SKIP appears
            # late. Fixed battle HUD icons establish battle independently;
            # their recognition never authorizes a click on an absent button.
            'treasure_battle_hud':{'box':[350,17,375,45], 'tile':[0,59,25,87],
                'atlas':'native_213124_controls', 'threshold':.93, 'strict':True, 'blur':True},
            'treasure_battle_skip':{'box':[27,494,122,520], 'tile':[0,91,95,117],
                'atlas':'native_213124_controls', 'threshold':.94, 'strict':True, 'blur':True,
                'alias':'battle_skip'},
        })
        for name,spec in native.items():
            if spec.get('reference_only'):
                target=name.removesuffix('_diagnostic')
                self.specs[target].setdefault('shape_variants',[]).append(name)
        ...
        atlases['native_213124_controls']=cv2.imdecode(np.fromfile(p/'native_213124_controls.png',np.uint8),cv2.IMREAD_COLOR)
        atlases['diagnostic_controls']=cv2.imdecode(np.fromfile(p/'diagnostic_controls.png',np.uint8),cv2.IMREAD_COLOR)
        if atlases['diagnostic_controls'] is None:raise ValueError('추가 일일 작업 인식 이미지가 없습니다.')
    ...
    def exposure(self,located):
        ...
        for names in EXPOSURE_ANCHORS:
            ...
            references=names[:1] if names in {('guild_dungeon_rank','guild_fight'),
                ('guild_dungeon_rank_diagnostic','guild_fight_diagnostic')} else (
            ...
            for references in reference_sets:
                ...
                # The separately captured opaque rank label includes native
                # rasterization. Repeated capture resizing attenuates its red
                # channel; only this independent label may use the wider fit.
                min_gain=.78 if references==('guild_dungeon_rank_diagnostic',) else .85
                if np.all((gain>=min_gain)&(gain<=1.40)) and np.all(np.abs(offset)<=45) and error<=8:
    ...
    def recognize(self,im):
        ...
        for name,(score,patch,xx,yy,ww,hh) in located.items():
            ...
            if name=='guild_loot_available':
                # The red notification is part of this full claim-button crop.
                # A grayscale/disabled button must not authorize a claim.
                good=good and bool(np.all(np.abs(patch.astype(float)-reference_color).mean(axis=(0,1))<=12))
                red=(patch[:,:,2].astype(float)-patch[:,:,1])>40
                good=good and int(red.sum())>=12
            if s.get('alias',name)=='guild_fight':
                reference_tint=float((reference_color[:,:,1]-reference_color[:,:,2]).mean())
                good=good and raw_color[1]-raw_color[2]>=max(2,reference_tint*.5)
            if s.get('alias',name) in {'d_enter','sweep_action'}:
