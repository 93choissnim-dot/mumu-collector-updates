...
class GameRecovery:
    ...
    def _recover(self):
        ...
        self.startup.clear();self.startup.update(pending=True,target=self.target,home=home,confirmed=set(),deadline=self.clock()+90)
    ...
    def confirm_startup(self,kind,point):
        ...
        try:self.device.click(fresh)
            ...
            raise
        self.log('재접속 / '+('다운로드 안내 확인' if kind=='download' else '오프라인 보상 확인'))
        return True
        ...
        while self.clock()<deadline:
            ...
            if foreground==self.target:
                ...
                if stable>=2 and screen.state in RECOVERY_READY:
                    ...
                    self.startup['pending']=False;self.observed_package=self.target
