...
from history import NO_AUTO_RETRY
...
def cycle_with_recovery(collector, selected, restore, adb, device, vision, stop, log,
                        ...
                        game_recovery=None):
    """Recover proven transport/app loss, retaining the same input ledgers."""
    ...
    recoveries = 0
    def resume_rooms():
        candidate=[r for r in selected if results.get(r) not in NO_AUTO_RETRY]
        if game_recovery is not None:
            from session_workflow import held_reason
            ledger=getattr(collector,'daily_ledger',None)
            records=ledger.snapshot(collector.daily_ident) if ledger is not None else {}
            for task in list(candidate):
                if held_reason(task,results.get(task),collector.action_state,records):
                    collector.record_task(task,'deferred');results[task]='deferred';candidate.remove(task)
        collector.last_image=collector.last_screen=None
        collector.forget_observations()
        collector.resume_counts={r:counts[r] for r in candidate if r in counts}
        return candidate
    while True:
        try:
            ...
            collector.last_image = collector.last_screen = None
            collector.forget_observations()
            remaining = [r for r in selected if results.get(r) not in NO_AUTO_RETRY]
            ...
            if adb.is_connected(device.serial):
                if game_recovery is not None and game_recovery.recover():
                    remaining=resume_rooms()
                    on_link(True)
                    continue
                raise
            on_link(False)
            ...
            if device.current_package() != expected:
                if game_recovery is None or not game_recovery.recover():
