...
class App(FleetCollection, FleetUI, Dashboard, Updates, RunControls):
    ...
    def save(self):
        ...
        value={**self.config,"adb_path":self.adb_path.get(),"address":self.address.get(),"serial":self.chosen_serial(),
               "selected":{k:v.get() for k,v in self.selected.items()},"minutes":self.minutes.get(),
               "restore_sleep":self.restore.get(),"players":self.players,"stop_hotkey":self.stop_hotkey.value,
    ...
    def clear_preview(self):
        self.preview_image=None
        self.preview_label.configure(image=self.preview_placeholder,text="선택한 뮤뮤의 화면을 확인해 주세요.")

    def on_device_changed(self, value=None):
        serial=self.chosen_serial()
        self.select_profile(serial)
        self.refresh_history()
        report=self.device_reports.get(serial)
        self.clear_preview()
        if report is None:
            self.connection_text.set("뮤뮤 창을 선택해 주세요.")
            self.device_info.set("뮤뮤 창에 지정한 이름으로 표시됩니다.")
            self.connection_badge.configure(text_color=GOLD)
            return
        package=report["package"]
        self.device_info.set("실행 앱: "+(package if len(package)<=46 else package[:43]+"…"))
        if report["image"] is None:
            self.connection_text.set("주소 연결됨 / 캡처 실패")
            self.connection_badge.configure(text_color=RED)
        else:
            self.preview_image=report["image"];self.render_thumbnail()
            known=report["state"] in READY_STATES
            app_known=package!="앱 미확인" and not report.get("package_error")
            if not app_known:
                self.connection_text.set("화면 캡처됨 / 실행 앱 확인 필요")
            else:
                self.connection_text.set("수령 준비됨" if known else "화면 캡처됨 / 수령 화면 확인 필요")
            self.connection_badge.configure(text_color=MINT if known and app_known else GOLD)
        if serial in self.unavailable_devices:
            self.connection_text.set("연결 재확인 필요 / 주소 유지")
            self.connection_badge.configure(text_color=GOLD)
    ...
    def refresh_history(self):
        serial = self.chosen_serial() or self.config.get("serial", "")
        for room in LABELS:
            if room not in self.card_history:continue
            entry = self.player_history(serial, room)
            self.card_history[room].configure(text=last_success(entry))
            if not self.busy() and self.selected[room].get() and entry.get("result") in RESULTS:
                self.card_notes[room].configure(text=RESULTS[entry["result"]])
    ...
    def poll(self):
        if os.name=="nt" and not self.hotkey_capture:
        ...
        try:
            while True:
                ...
                if kind.startswith("update_"):
                    ...
                    line=time.strftime("%H:%M:%S")+"  "+value+"\n"
