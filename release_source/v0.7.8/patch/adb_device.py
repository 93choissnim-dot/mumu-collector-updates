...
READY_STATES = frozenset({"main", "menu", "sleep", "equipment", "offline_reward"} |
...
class Adb:
    ...
    def run(self, args, timeout=10, input_generation=None):
        ...
        is_input = 'input' in args and 'shell' in args
        if not is_input:
...
class AdbDevice:
    ...
    def command(self, args, timeout=10):
        if isinstance(self.adb, Adb) and 'input' in args and 'shell' in args:
    ...
    def bind_game(self, vision):
        ...
        if screen.state not in READY_STATES:
