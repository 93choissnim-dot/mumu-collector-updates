    ...
    def launch(self,mode):
        ...
        try:
            ...
            for ident,p in self.players.items():
                ...
                jobs.append(job)
            if not jobs:raise Halt('세팅 설정에서 수령할 창을 체크해 주세요.')
            self.save()
        ...
        def execute(job,rooms):
            ...
            def recorded(room,result):
                results[room]=result
                try:self.history.record(ident,room,result)
            ...
            try:
                ...
                collector=ExtraCollector(device,vision,self.stop,log,on_frame=observed,
                    on_progress=lambda text:emit('progress',job['name']+' / '+text),
                    on_result=recorded,on_issue=lambda room,reason:capture(reason,room))
                return cycle_with_recovery(collector,rooms,job.get('restore_sleep',True),adb,device,vision,
                ...
                for room in rooms:
                    if room not in results:recorded(room,'failed')
                return results
