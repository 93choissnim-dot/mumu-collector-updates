...
class RecoveryTests(unittest.TestCase):
    def run_start(self, initial, *, wake_to='menu', back_to=None, cancel_to='main', stuck=False, stop_on=None, partial=False, cls=ExtraCollector):
        ...
        def click(point):
            ...
            elif d.state.startswith(('farm_','wood_','mine_')):
                self.assertEqual(point,(34,28));action('room_back','main')
    ...
    def test_known_page_starts_with_back(self):
        for page in ['ranking','training','boss_rank']:
            d,r=self.run_start(page)
            self.assertEqual(r.state,'menu');self.assertEqual(d.actions,['back','menu'])

    def test_known_facility_uses_back_arrow_without_escape(self):
        for page in ('farm_ready','wood_empty','mine_wait'):
            d,r=self.run_start(page)
            self.assertEqual(r.state,'menu');self.assertEqual(d.actions,['room_back','menu'])
