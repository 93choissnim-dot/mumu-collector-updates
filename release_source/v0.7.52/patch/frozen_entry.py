...
def health_check(output,full_ui=True):
    ...
    suite=unittest.TestSuite(unittest.defaultTestLoader.loadTestsFromTestCase(case)
        for case in (CombatStartTests,NativeDungeonTests,NativeRaidTests,DungeonConfirmationTests,CompletionRecheckTests,CompletionReviewUITests,CompletionReviewFleetTests,HistoryCountTests,PendingHistoryTests,RaidRetryTests,InputProvenanceTests,InputEvidenceTests,NativeAutumnTests,NativeNavigationTests,RetryResolutionTests,HistoryLifecycleTests,RewardRecoveryTests,UpdateDiagnosticsTests,
