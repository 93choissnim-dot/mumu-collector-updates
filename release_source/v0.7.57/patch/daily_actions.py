...
class DailyActions(DailyExecution):
    ...
    def daily_can_resume_pending(self):
        if self.daily_task!='daily_dungeons' or getattr(self,'daily_verification_only',False):return False
        detail=self.daily_detail();saved=getattr(self,'dungeon_resume_requests',{}).get(self.daily_step)
    ...
    def daily_retire_inherited_pending(self):
        ...
        self.daily_checkpoint('uncertain',pending=None,retry_authorized=True,
                              resolution_source='fresh_dungeon_run',reason='이전 결과 미확인 기록 보존 / 현재 횟수 재확인')
        self.dungeon_resume_requests.pop(self.daily_step,None)
