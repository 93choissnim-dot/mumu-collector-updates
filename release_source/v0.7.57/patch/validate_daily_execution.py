...
class DailyExecutionTests(unittest.TestCase):
    ...
    def test_repaired_recognition_block_is_rechecked_once_and_persisted(self):
        ...
        c.daily_checkpoint('blocked',reason=reason,failures=2,recognition_revision=40)
        action=Mock(side_effect=Halt(reason))
        c.daily_run_step('treasure','보물 창고',action)
        self.assertEqual(action.call_count,1)
        self.assertEqual(c.daily_detail()['recognition_revision'],54)
        c.daily_run_step('treasure','보물 창고',action)
    ...
    def test_legacy_started_raid_cannot_complete_without_a_result(self):
        c=self.c;c.daily_task='daily_guild';c.daily_step='raid';c.daily_mark('raid','started')
        c.daily_guild_page=Mock();c.daily_tap=Mock()
        c.daily_wait=Mock(side_effect=[screen('daily_raid_map'),screen('daily_raid_detail','raid_fight')])
        c.daily_combat=Mock(return_value=(screen('daily_raid_map'),False))
        with self.assertRaises(OutcomeUnknown):c.daily_guild_raid()
        self.assertFalse(c.daily_done('raid'))
        self.assertEqual([a.args[1] for a in c.daily_tap.call_args_list if len(a.args)>1].count('raid_fight'),1)
    def test_deferred_is_visible_issue_but_has_no_automatic_retry(self):
        ...
        self.assertEqual(h.stats('vm')['held_tasks'],1);self.assertEqual(h.stats('vm')['issue_tasks'],1)
...
class ReadinessTests(unittest.TestCase):
    ...
    def test_page_and_button_confirm_together_but_tap_still_requires_fresh_frame(self):
        ...
        c=self.collector([ready,ready,ready])
        ...
        c.daily_tap(found,'sweep_action');self.assertEqual(c.screen.call_count,3)
...
class DurableEntryTests(unittest.TestCase):
    ...
    def test_pending_entry_stays_unconfirmed_on_exhaustion_without_reentering(self):
        ...
        with self.assertRaises(OutcomeUnknown):c.daily_dungeon('treasure')
        self.assertFalse(c.daily_done('treasure'));self.assertEqual(c.daily_detail()['pending'],'d_enter')
        c.device.click.assert_not_called()
    def test_pending_sweep_never_replays_on_unchanged_button(self):
        ...
        s=screen('daily_sweep','sweep_action');c.daily_find_dungeon=Mock(return_value=screen('daily_room_equipment','d_sweep_open'));c.daily_tap=Mock()
