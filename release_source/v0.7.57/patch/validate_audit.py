...
class FacilityRecoveryTests(unittest.TestCase):
    ...
    def test_unconfirmed_claim_remains_pending_without_repeat(self):
    ...
    def test_manual_retry_reopens_pending_free_facility_once(self):
        for room in ('farm','wood','mine'):
            with self.subTest(room=room),tempfile.TemporaryDirectory() as tmp:
                c,d,seen=self.collector()
                path=Path(tmp)/'actions.json';c.action_state=ActionState(path,'vm')
                c.action_state.reserve(room)
                c.manual_retry_rooms={room}
                self.assertEqual(c.cycle([room]),{room:'collected'})
                self.assertEqual(d.claims[room],1)
                self.assertFalse(ActionState(path,'vm').pending(room))
                self.assertEqual(seen,[(room,'collected')])
    def test_manual_retry_that_stays_active_keeps_pending_and_cannot_repeat(self):
        c,d,seen=self.collector(post={'wood':'ready'})
        c.action_state.reserve('wood');c.manual_retry_rooms={'wood'}
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(d.claims['wood'],1)
        self.assertTrue(c.action_state.pending('wood'))
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(d.claims['wood'],1)
    def test_manual_retry_without_old_pending_cannot_repeat_new_uncertain_input(self):
        c,d,seen=self.collector(post={'wood':'ready'});c.manual_retry_rooms={'wood'}
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(d.claims['wood'],1)
    def test_manual_retry_empty_reconciles_without_input(self):
        c,d,seen=self.collector(initially_empty=True)
        c.action_state.reserve('wood');c.manual_retry_rooms={'wood'}
        self.assertEqual(c.cycle(['wood']),{'wood':'collected'})
        self.assertEqual(d.claims['wood'],0)
        self.assertFalse(c.action_state.pending('wood'))
    def test_manual_retry_unreadable_button_preserves_pending_without_input(self):
        c,d,seen=self.collector(initial_modes={'wood':'wait'})
        c.action_state.reserve('wood');c.manual_retry_rooms={'wood'}
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(d.claims['wood'],0)
        self.assertTrue(c.action_state.pending('wood'))
    def test_manual_retry_does_not_authorize_another_room(self):
        c,d,seen=self.collector();c.action_state.reserve('wood');c.manual_retry_rooms={'farm'}
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(d.claims['wood'],0)
    def test_manual_retry_resume_count_cannot_authorize_a_second_dispatch(self):
        c,d,seen=self.collector();c.action_state.reserve('wood')
        c.manual_retry_rooms={'wood'};c.resume_counts={'wood':1}
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertEqual(d.claims['wood'],0)
    def test_pending_input_from_previous_run_is_not_logged_as_new_click(self):
        c,d,seen=self.collector();logs=[];c.log=logs.append;c.action_state.reserve('wood')
        self.assertEqual(c.cycle(['wood']),{'wood':'deferred'})
        self.assertTrue(any('이번 실행 0회 클릭' in line for line in logs),logs)
...
class SweepAndWorkshopTests(unittest.TestCase):
    ...
    def sweep(self,key,ready,fresh,*,confirmed_sweep=False):
        ...
        observations=iter(ready)
        def observe():
            value=next(observations)
            # This regression isolates pre-input rechecks. A completed sweep
            # now also needs reward proof; missing-proof cases live separately.
            if confirmed_sweep and c.daily_detail().get('pending')=='sweep_action':c.daily_reward_seen=True
            return value
        c.daily_ready=Mock(side_effect=lambda *args,**kwargs:observe());c.screen=Mock(side_effect=[room]+fresh)
        c.daily_dungeon(key)
    ...
    def test_transient_effect_rechecks_then_sweeps_once(self):
        ...
        self.sweep('equipment',[free,active,active,done],[page('unknown'),free,free,active,done],confirmed_sweep=True)
