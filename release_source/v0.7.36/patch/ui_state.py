...
def task_display_order(player,entries,show_disabled=False):
    ...
    selected=set(selected_tasks(player))
    def priority(task):
        if task not in selected:return 3
        result=entries.get(task,{}).get('result')
        if result in ISSUES:return 0
        if result is None:return 1
        return 2
    return sorted((task for task in TASK_LABELS if task in selected or show_disabled),key=priority)
