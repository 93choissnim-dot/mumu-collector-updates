...
class Dashboard:
    def build(self):
        ...
        self.toolbar=ctk.CTkFrame(self.root,fg_color=PANEL,corner_radius=0)
        ...
        identity.grid(row=0,column=0,sticky='ew',padx=(24,10),pady=(14,12));identity.grid_columnconfigure(0,weight=1)
        self.detail_name=heading(identity,'뮤뮤를 연결하세요',size=24,anchor='w')
        self.detail_name.grid(row=0,column=0,sticky='w')
        self.connection_badge=label(identity,textvariable=self.connection_text,size=11,color=MUTED,anchor='w')
        self.connection_badge.grid(row=1,column=0,sticky='w')
        ...
        detail=panel(body);self.detail_panel=detail;detail.grid_columnconfigure(0,weight=1);detail.grid_rowconfigure(2,weight=1)
        ...
        for col,title in enumerate(['현재 상태','다음 수령','수령 간격']):
            ...
            value=label(box,'—',size=21,bold=True);value.pack(anchor='w',pady=(3,0));self.detail_summary.append(value)
        ...
        for key,title in TASK_LABELS.items():
            row=ctk.CTkFrame(self.detail_tasks,fg_color=INSET,corner_radius=7);row.grid_columnconfigure(1,weight=1)
            label(row,'',image=icon(key,color=ACCENT,size=22),width=26,height=32).grid(row=0,column=0,padx=(12,9),pady=5)
            label(row,title,size=13,bold=True,anchor='w').grid(row=0,column=1,sticky='w',padx=(0,10))
            result=label(row,'기록 없음',size=12,color=MUTED,width=112,anchor='w');result.grid(row=0,column=2,sticky='w',padx=(4,12))
        ...
        self.stop_button=button(footer,'중지',self.stop_run,width=158,height=44,state='disabled',fg_color=RED,hover_color='#8D3E37',text_color=CREAM)
    ...
    def sync_run_actions(self,busy=None):
        ...
        self.run_info.grid_configure(row=0,column=0,columnspan=3 if compact else 1)
        self.status_label.grid_configure(row=1,column=0,columnspan=3 if compact else 1)
        self.shortcut_hint.grid_configure(row=2,column=0,columnspan=3 if compact else 1)
    ...
    def layout_task_cards(self):
        ...
        visible=task_display_order(p,self.history_entries(self.view_id),self.show_disabled_tasks) if p else []
        ...
        if signature!=self._task_layout:
            ...
            for i,key in enumerate(visible):
                self.detail_task_rows[key].grid(row=i,column=0,sticky='ew',padx=4,pady=2)
