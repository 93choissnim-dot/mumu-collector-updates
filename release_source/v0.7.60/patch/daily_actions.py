...
class DailyActions(DailyExecution):
    ...
    def daily_wait_sweep_action(self,page,timeout=25):
        ...
        end=self.now()+timeout;reopens=0
        while self.now()<end:
            screen=self.daily_ready({'daily_sweep',page},['sweep_action','d_sweep_open'],
                                    timeout=max(1,end-self.now()))
            if screen.state=='daily_sweep' and self.daily_has(screen,'sweep_action'):return screen
    ...
    def daily_guild_donation(self):
        ...
        for attempt in range(5):
            ...
            if self.daily_has(screen,'donate_free'):self.daily_tap(screen,'donate_free')
                ...
                self.daily_tap(screen,'donate_close')
