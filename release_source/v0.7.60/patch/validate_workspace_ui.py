...
def layout_evidence(app):
    root=app.root
    constraints={name:tuple(map(int,root.tk.splitlist(root.tk.call('wm',name,root._w))))
                 for name in ('minsize','maxsize')}
    expected={name:tuple(root._apply_window_scaling(getattr(root,'_'+bound+'_'+axis)) for axis in ('width','height'))
              for name,bound in (('minsize','min'),('maxsize','max'))}
    return {'geometry':root.tk.call('wm','geometry',root._w),
            'size':(root.winfo_width(),root.winfo_height()),
            'window_scale':root._get_window_scaling(),'widget_scale':app.main_panel._get_widget_scaling(),
...
def settle_compact_geometry(app,timeout=3):
    # CTk 5.2.2 restores scaling-time min/max locks after 1000 ms. update()
    # alone does not await that timer; request the test size after it releases.
    root=app.root;deadline=time.monotonic()+timeout;requested=False
    while time.monotonic()<deadline:
        root.update();state=layout_evidence(app)
        if state['constraints']==state['expected_constraints']:
            if not requested:
                root.geometry('980x640+0+0');requested=True
            app.apply_layout();root.update();state=layout_evidence(app)
            expected=(round(980*state['window_scale']),round(640*state['window_scale']))
            if (state['constraints']==state['expected_constraints'] and state['size']==expected and
                    root._parse_geometry_string(state['geometry'])==(*expected,0,0) and state['compact']):
                return
        time.sleep(min(.01,max(0,deadline-time.monotonic())))
    raise AssertionError(('compact geometry did not settle',layout_evidence(app)))
...
def check(app,output):
    ...
    app.history_window.destroy()
    assert app.display_task_order[0]=='ranking'
    ...
    for widget in [app.start_button,app.once_button,app.scope_selector,app.inspect_button]:
    ...
    for scale in (1.25,1.5):
        ...
        for widget in [app.start_button,app.once_button,app.scope_selector,app.update_button,app.more_button]:
    ...
    for geometry in ('680x430+0+0','820x520+0+0'):
        ...
        assert not app.roster_panel.winfo_ismapped() and not app.fleet_bar.winfo_ismapped()
        for widget in (app.start_button,app.once_button,app.scope_selector,app.inspect_button,app.history_button):
    ...
    assert app.once_button.cget('state')=='disabled'
