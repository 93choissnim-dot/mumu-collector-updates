...
class Collector:
    ...
    def click_match(self, state, name, screen=None):
        # Reuse only this collector's latest confirmed frame. AdbDevice
        # also checks the current app and capture age immediately before touch.
        ...
        self.trace.event('input',action=name,point=list(screen.matches[name].center),before=screen.state)
        self.device.click(screen.matches[name].center)
        self.pause(.6)
    ...
    def collect_room(self, room, screen=None):
        ...
        for attempt in range(self.claim_counts[room]+1, self.CLAIM_ATTEMPTS+1):
            ...
            self.click_match(screen.state, button, screen)
            self.claim_counts[room] += 1
